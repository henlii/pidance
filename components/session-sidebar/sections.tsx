 ...
 * ProjectSection / SessionTreeItem / SessionItem 从 SessionSidebar.tsx 迁出；
 * props 契约保持不变，纯渲染无状态逻辑。
 */
...
import { type SidebarProjectNode } from "../session-sidebar-model";
import {
...
export { ProjectSection, SessionTreeItem, SessionItem };
function ProjectSection({
  ...
  /** 行内动作被拒绝时的提示（如「项目里还有运行中的会话」）；无则不渲染。 */
  ...
  const hasSessions = project.tree.length > 0;
    ...
    <div>
      ...
      {!collapsed && (
        <div>
          {getVisibleTopLevelNodes(
            project.tree,
            getGroupVisibleCount(groupVisibleCounts, project.root),
            searchActive,
          ).map((node) => (
            <SessionTreeItem
              key={node.session.id}
          ...
          <GroupPagination
            groupKey={project.root}
            total={project.tree.length}
            visibleCount={getGroupVisibleCount(groupVisibleCounts, project.root)}
            searchActive={searchActive}
