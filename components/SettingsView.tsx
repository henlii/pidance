...
function GeneralPage() {
  ...
  const [runtimeResolved, setRuntimeResolved] = useState<{
    path: string | null;
    version: string | null;
    source: string;
  } | null>(null);
  ...
  return (
    <div className="settings-page-content">
      ...
      <div style={{ marginBottom: 22 }}>
        <div style={sectionTitle}>{t("general_runtimeSection")}</div>
        <div style={{ fontSize: 11, color: "var(--text-dim)", lineHeight: 1.6, marginBottom: 12, maxWidth: 520 }}>
          {t("general_runtimeHint")}
        </div>
        <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>{t("general_runtimeDir")}</div>
        <input
          type="text"
          value={runtimeDir}
          onChange={(e) => setRuntimeDir(e.target.value)}
          placeholder={t("general_runtimeDirPlaceholder")}
          spellCheck={false}
          style={{
            width: "100%",
            maxWidth: 520,
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center", marginBottom: 10 }}>
            ...
            onClick={() => {
              ...
              void (async () => {
                try {
                  ...
                  if (!res.ok) {
                    setRuntimeMsg(data.error || `HTTP ${res.status}`);
                    return;
                  }
                  setRuntimeDir(typeof data.runtimeDir === "string" ? data.runtimeDir : "");
                  setRuntimeResolved(data.resolved ?? null);
          ...
          >
            {runtimeBusy ? t("general_runtimeSaving") : t("general_runtimeSave")}
          </button>
          <button
            type="button"
            ...
            onClick={() => {
              ...
              void (async () => {
                try {
                  const res = await fetch("/api/runtime/install-pi", { method: "POST" });
                  const data = (await res.json()) as {
                    ok?: boolean;
                    message?: string;
                    path?: string | null;
                  ...
                  setRuntimeMsg(data.message || (res.ok ? "ok" : `HTTP ${res.status}`));
                  if (data.path) {
                    setRuntimeResolved({
                      path: data.path,
                  ...
                  // 刷新配置展示
                  const cfg = await fetch("/api/runtime/config", { cache: "no-store" });
                  if (cfg.ok) {
                    const j = (await cfg.json()) as {
                      runtimeDir?: string;
                      resolved?: { path: string | null; version: string | null; source: string };
                    };
                    if (typeof j.runtimeDir === "string") setRuntimeDir(j.runtimeDir);
                    if (j.resolved) setRuntimeResolved(j.resolved);
                  }
                } catch (e) {
                  setRuntimeMsg(e instanceof Error ? e.message : String(e));
                } finally {
            ...
            style={{
              ...
              fontSize: 12,
              fontWeight: 600,
              opacity: installBusy ? 0.75 : 1,
            }}
          >
            {installBusy ? t("general_installingPi") : t("general_installPi")}
          </button>
        </div>
        <div style={{ fontSize: 11, color: "var(--text-dim)", lineHeight: 1.5, maxWidth: 520, marginBottom: 8 }}>
          {t("general_installPiHint")}
        </div>
        {runtimeResolved?.path ? (
          <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-mono)", wordBreak: "break-all", maxWidth: 520 }}>
            {t("general_runtimeResolved", {
              path: runtimeResolved.path,
              version: runtimeResolved.version || "?",
            })}
          </div>
        ) : (
          <div style={{ fontSize: 11, color: "var(--warning)" }}>{t("general_runtimeMissing")}</div>
        )}
        {runtimeMsg && (
          <div style={{ marginTop: 8, fontSize: 11, color: "var(--text-muted)", whiteSpace: "pre-wrap", maxWidth: 520 }}>
            {runtimeMsg}
          </div>
        )}
      </div>
