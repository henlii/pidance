...
> 当前版本：`0.2.37` · npm 包：`@henlii/pidance` · CLI：`pidance`

## 设计原则

**兼容优先，然后在兼容之上做通用的显示优化。**

1. **内容完整优先**：Pidance 的目标是在浏览器里体验原生 TUI 会话的内容，**包括扩展（插件）自定义的界面**。原生 TUI 里能显示的东西，Pidance 都要能显示；宁可先原样呈现，也不要因为「Web 里不好看」而少显示。
2. **兼容高于观感**：插件用 TUI 扩展 API 表达的内容（自绘组件、overlay、widget、状态条、对话框）都必须有落点。暂不具备的能力按**可见方式**降级——给出提示或明确返回值，不静默丢弃（静默会让插件作者以为生效了）。
3. **优化必须通用**：显示层的改进（排版、折叠、卡片壳、标题、无障碍）要走所有插件共用的机制，不为个别插件写特例。插件专属代码只允许出现在兼容层：解析它发的私有载荷、把 TUI 语义桥到 Web。
4. **Pi 仍是事实来源**：会话文件、工具执行、压缩与分支语义由 Pi SDK 决定；Pidance 不复制、不覆盖这些语义，只负责投影与呈现。界面分区对照见 [`docs/ui-vs-tui.md`](./docs/ui-vs-tui.md)。
