...
Use the worktree switcher to choose which checkout Pidance should use for new work in that project.
...
Pidance creates the checkout at:
...
- The Git branch.
- Pidance session history.
- The main checkout.

If the worktree has uncommitted or untracked files, Git refuses the removal. Pidance then offers a force remove action. Force removal discards the uncommitted files in that checkout, so use it only when you no longer need those changes.

## Sessions and Worktrees

Pidance groups sessions by project root, so sessions from the main checkout and linked worktrees appear together.
...
Git can keep prunable worktree records after a checkout disappears. Pidance filters those out of the switcher.
