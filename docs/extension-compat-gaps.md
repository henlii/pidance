...
| 17 | widget 组件的 `handleMouse` 永不调用、`move/drag/wheel` 不转发 | `lib/web-extension-ui.ts` `inputCustomMouse` 只查 custom 面板 | pi-subagents（第 0 行左键把**整块**在「一行摘要/全列表」间切换） | 见右 | **刻意分叉**（#76）：Web 的折叠由共用卡片头承担（粒度是卡片级 vs 整块二选一），方向键 roster 已由 #83 打通 |
