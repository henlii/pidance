...
| 15 | `getEditorComponent()` 恒 `undefined`（即使刚 `setEditorComponent` 成功），「包裹上一个编辑器」模式断链 | `lib/web-extension-ui.ts` | 「包裹上一个编辑器」模式 | 存下工厂并回传 | **已修**（#74 存下并回传；#107 真的在输入框位置渲染）：工厂值如实回传（SDK 契约 = 当前**配置的**工厂，未配置才是 undefined）；#107 起 Web 会用它在**输入框位置**接管渲染（桌面、可写、设置里没关掉时），按键进 `handleInput`、`onSubmit` 走既有发送管线，不再发降级提示；接管中我们的输入框整块让位，壳外有「返回输入框」按钮 |
...
| 22 | `Image` 组件 / 终端图片协议无落点 | `lib/kitty-image.ts` + `components/RenderedLines.tsx` + `lib/tui-render-bridge.ts` | 已装插件 0 使用 | 客户端画真 `<img>` | **已修**（#104）**部分覆盖**：渲染期间临时把 `getCapabilities().images` 置成 `kitty`（渲染后还原，进程全局不能长期置位），渲染桥**先摘图、再判文本上限**（一张图的 base64 轻易超过单行 4000/合计 200KB，先校验会把整段渲染连文本一起丢掉），摘出的图片随结果下发、客户端按 `rows × 实测行高` 预留高度画真 `<img>`（`alt` 来自 i18n）。**已有图片的位置**：custom 面板、widget 组件。**仍是可见降级（文本说明）**：工具卡 / 页头页脚槽位 / entry 与消息投影行（后两者的投影类型是 `string[]`，要带图片得改读写视图的形状 —— 待父级开 issue 跟踪）；摘不出图的（超 4MB、非 PNG、分块不完整、空载荷）一律降级成一句说明，不静默丢。 |
