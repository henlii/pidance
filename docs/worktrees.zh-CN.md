...
worktree 切换器决定 Pidance 接下来使用哪个 checkout。
...
如果 worktree 里有未提交或未跟踪文件，Git 会拒绝删除。Pidance 会再显示 force remove。force remove 会丢弃这个 checkout 里的未提交文件，只在确定不需要这些改动时使用。

## 会话和 Worktree 的关系

Pidance 按 project root 分组会话，所以 main checkout 和 linked worktree 里的会话会显示在一起。
...
Git 有时会保留 prunable worktree 记录。Pidance 会过滤这些记录，不在切换器里显示。
