...
- **残留**（未纳入 #97 范围，另开 issue #109）：已下发的**消息投影行**（`renderedLines`）在切主题后仍是旧色，直到页面重载或重新投影。
  已修（#109）：切主题时按 entryId 把那批行重取一遍（不重拉一页、不动窗口与滚动位置），
  并按**会话 slot** 记窗口形态与「被替换掉的旧行」——跳读窗口显式 `toEnd=0`（路由缺省是取到叶尾），
  换色后落地的在途响应若带的是同一内容，套回新色；内容真的变了则让磁盘赢。
  已知取舍：一次最多 500 行、跳数有限，已加载窗口特别大时中间会留一段旧色（无提示，等下一次整段 replace）。
