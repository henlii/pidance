...
6. **`onTerminalInput`（插件全局按键）有两条窄道**：pi-tui 的 `addInputListener` 是全局同步的，Web 没有等价层，**也不做「每个键都往返」**。现在只有两种情形会把按键 POST 给 `terminal_input`（服务端按 pi-tui 语义逐个调监听器：先 `consume` 再 `data` 改写，见 `dispatchTerminalInput`）：
   ...
   **注册时会告知覆盖范围**（issue #74）：`onTerminalInput` 落地时发一条只发一次的 warning（`lib/web-extension-ui.ts` 的 `notifyLimitedSupport`），写明「只在这两个窄窗口里收得到按键、普通打字到不了」。否则插件无从区分「用户没按」和「Web 端收不到」，这块交互会静默消失 —— 但也不能写成「不支持」：按键确实会送达，只是覆盖面窄。
...
10. **仍存在的差异**：
   ...
   - **`getEditorText()` 回传的是「已同步到服务端的输入框草稿」**（2026-09-25，issue #74）：宿主把 `lib/composer-draft-text.ts` 的读取器注入适配器，读的是客户端**本来就会**镜像到 `/api/preferences` 的那份草稿（`lib/draft-store.ts`，草稿键就是会话 id）—— **零新增往返**，代价是一次小文件读（实测 ~0.6ms）。语义边界：比输入框**滞后**（客户端 400ms 防抖 + 落盘），多标签共用同一个草稿键（最后写入者胜出），所以是「这个会话的草稿」而不是「某个标签页的编辑器」；没有草稿 / 读失败一律空串。pi-subagents 的激活门槛（`ctx.ui.getEditorText() === ""`）另由**客户端**把关（空输入框才路由），两条判据互不依赖。
