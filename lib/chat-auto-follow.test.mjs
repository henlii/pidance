...
test("shouldPinAfterRunSettle：只在 following 才钉底，released 不写 scrollTop", () => {
  assert.equal(shouldPinAfterRunSettle("following"), true);
  assert.equal(shouldPinAfterRunSettle("released"), false);
});
...
// 从后台切回前台常伴随一次落在正文上的单击（不是拖选）；若 pointerdown 当场释放跟随，
// 用户看到的就是「切一下窗口自动滚动就停了」。
test("isPointerSelectIntent：单击不算拖选意图，位移达标才算", () => {
  const origin = { x: 100, y: 200 };
  // 原地点下/抬起（含 1-2px 抖动）：不是拖选
  assert.equal(isPointerSelectIntent(origin, { x: 100, y: 200 }), false);
  assert.equal(isPointerSelectIntent(origin, { x: 102, y: 201 }), false);
  // 达到阈值：拖选
  assert.equal(isPointerSelectIntent(origin, { x: 100 + POINTER_SELECT_INTENT_PX, y: 200 }), true);
  assert.equal(isPointerSelectIntent(origin, { x: 100, y: 200 - POINTER_SELECT_INTENT_PX - 1 }), true);
  assert.equal(isPointerSelectIntent(origin, { x: 100 + POINTER_SELECT_INTENT_PX, y: 200 + POINTER_SELECT_INTENT_PX }), true);
});

test("useChatAutoFollow 收尾钉底在回调里再查 shouldPinAfterRunSettle", () => {
  const source = readFileSync(fileURLToPath(new URL("../hooks/useChatAutoFollow.ts", import.meta.url)), "utf8");
  assert.match(source, /pendingEndPinRef\.current = shouldPinAfterRunSettle\(autoFollowModeRef\.current\)/);
  assert.match(source, /if \(!shouldPinAfterRunSettle\(autoFollowModeRef\.current\)\) return;/);
  assert.match(source, /if \(mode === "released"\) pendingEndPinRef\.current = false;/);
});
