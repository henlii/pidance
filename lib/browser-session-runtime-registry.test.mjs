...
test("标签页隐藏超阈值关闭本标签 SSE；阈值内回前台取消；回前台重连重建（issue #86）", async () => {
  const streamIds = [];
  const closedIds = [];
  let timerSeq = 0;
  const liveTimers = new Map();
  ...
  /**
   * 定时器延迟必须被断言，而且要比的是**拍板字面值**：
   * - 只收回调（不看 delay）= 阈值完全不受保护；
   * - 断言拿模块里的同一个常量 = 改常量照样绿，同样锁不住。
   * 这里写 5 * 60 * 1000 是有意的：它就是「隐藏多久关流」这个产品取值。
   */
  const HIDDEN_CLOSE_DECIDED_MS = 5 * 60 * 1000;
  const assertHiddenCloseDelays = () => {
    assert.ok(scheduledDelays.length > 0, "隐藏后必须调度过关闭定时器");
    assert.deepEqual(
      [...new Set(scheduledDelays)],
      [HIDDEN_CLOSE_DECIDED_MS],
      "隐藏关流必须用拍板的 5 分钟阈值，而不是空闲窗口那 5 秒",
    );
  };

  const sub = registry.attach("A");
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.deepEqual(streamIds, ["A"], "attach 建立事件流");

  // 隐藏：只挂定时器，不到点不关（阈值是 5 分钟，不是 idle 的 5 秒）
  registry.setTabVisibility(false);
  assert.equal(liveTimers.size, 1, "隐藏后挂上关流定时器");
  assert.deepEqual(closedIds, [], "未到阈值不关流");

  // 阈值内回前台：取消待执行的关闭，流保留
  registry.setTabVisibility(true);
  assert.equal(liveTimers.size, 0, "回前台取消待执行的关闭");
  fireAll();
  assert.deepEqual(closedIds, [], "阈值内不关流");

  // 再次隐藏并等到点：本标签的流被关掉，但 slot/时间线保留
  registry.setTabVisibility(false);
  assertHiddenCloseDelays();
  fireAll();
  assert.deepEqual(closedIds, ["A"], "阈值到期关闭本标签的流");
  assert.equal(registry.getEventSource("A"), null, "关流后无残留 source");
  assert.equal(registry.hasActiveEventStream("A"), false);

  // 回前台：既有激活路径（ensureEventsConnected）重建连接
  registry.setTabVisibility(true);
  registry.ensureEventsConnected("A");
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(streamIds.length, 2, "回前台重连重建事件流");
  assert.ok(registry.getEventSource("A"), "重连后重新有 source");
  assert.equal(liveTimers.size, 0, "重建后不残留隐藏关闭定时器");
  sub.dispose();
});

test("隐藏期间不新建连接；回前台才重建（issue #86）", async () => {
  const streamIds = [];
  const closedIds = [];
  let timerSeq = 0;
  const liveTimers = new Map();
  ...
  // 已隐藏状态下才建立连接（手机冻结后重建页面）：不建立，也不挂定时器
  registry.setTabVisibility(false);
  const sub = registry.attach("A");
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.deepEqual(streamIds, [], "隐藏期间不得新建连接");
  assert.equal(liveTimers.size, 0, "没有流就不需要关流定时器");

  // 隐藏期间的轮询/重建路径也不能把流接回来
  registry.ensureEventsConnected("A");
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.deepEqual(streamIds, [], "隐藏期间 ensureEventsConnected 也不得新建连接");

  // 回前台：激活路径重连
  registry.setTabVisibility(true);
  registry.ensureEventsConnected("A");
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.deepEqual(streamIds, ["A"], "回前台才重建连接");
  assert.deepEqual(closedIds, [], "全程没有关过流");
  sub.dispose();
});
...
test("G1 补充：只拿到 connected（带本轮序号）的重连，本轮 agent_end 必须收尾", () => {
  const stream = fakeStream();
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt() { throw new Error("unused"); },
    createEventStream: () => stream.manager,
  ...
  // 页面重连到第 2 轮：首帧只有 connected（+快照帧），没有 agent_start。
  registry.applyEvent("A", { type: "connected", isStreaming: true, streamRunSeq: 2 });
  assert.equal(registry.getRunState("A").agentRunning, true, "isStreaming 先对齐运行态");

  // 本轮收尾：序号相符 → 必须落下，不能被当成「迟到的上一轮」丢掉。
  registry.applyEvent("A", { type: "agent_end", streamRunSeq: 2 });
  assert.equal(registry.getRunState("A").agentRunning, false, "重连后运行态必须落下来");

  // 连 agent_start 都没收到（只知旧序号）时收到更新的序号：同样必须收尾。
  registry.applyEvent("A", { type: "connected", isStreaming: true, streamRunSeq: 3 });
  registry.applyEvent("A", { type: "agent_end", streamRunSeq: 4 });
  assert.equal(registry.getRunState("A").agentRunning, false, "序号更大说明漏了 agent_start，不能卡住");
...
test("connected{isStreaming}：先对齐运行态，首帧快照的 message_update 不被当过期帧丢掉", () => {
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt() { throw new Error("unused"); },
    createEventStream: () => fakeStream().manager,
    restoreDraft() {},
  });
  // 服务端首帧：本轮确实在流式，且已经生成了一段部分回复。
  registry.applyEvent("A", { type: "connected", isStreaming: true });
  registry.applyEvent("A", {
    type: "message_update",
    message: { role: "assistant", content: [{ type: "text", text: "partial" }] },
  });
  const snap = registry.getSnapshot("A");
  assert.equal(snap.agentRunning, true, "connected 必须先把运行态对齐");
  assert.equal(snap.streamState.isStreaming, true);
  assert.ok(
    JSON.stringify(snap.streamState.streamingMessage ?? null).includes("partial"),
    "快照里的部分回复必须立刻落到流式态（不能等下一个 chunk）",
  );
});

test("connected 不带 isStreaming（空闲会话）不得把会话弄成运行中", () => {
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt() { throw new Error("unused"); },
    createEventStream: () => fakeStream().manager,
    restoreDraft() {},
...
// ── issue #109：主题刷新换色（不是重拉一页） ──────────────────────────────────

const themeCustom = (lines) => ({ role: "custom", customType: "qa", content: "probe", display: true, renderedLines: lines });
const themeUser = (content) => ({ role: "user", content, timestamp: 1 });

/** 三页时间线：先尾页，再 prepend 更早页，再 tail 重拉（与真实滚动一致）。 */
function themeRegistry() {
  ...
  registry.hydrate("T", [themeCustom(["old-5"]), themeUser("u6")], ["e5", "e6"], { mode: "replace" });
  registry.hydrate("T", [themeCustom(["old-1"]), themeUser("u2")], ["e1", "e2"], { mode: "prepend" });
  registry.hydrate(
    "T",
    [themeCustom(["old-1"]), themeUser("u2"), themeCustom(["old-5"]), themeUser("u6")],
    ["e1", "e2", "e5", "e6"],
    { mode: "tail" },
  );
  return registry;
...
test("主题刷新：更早页的 renderedLines 也换新色，且窗口/长度不被换成尾页", () => {
  const registry = themeRegistry();
  const before = registry.getSnapshot("T");
  assert.deepEqual(before.entryIds, ["e1", "e2", "e5", "e6"], "前置条件：四页都在时间线里");
  const published = [];
  registry.subscribe("T", (snapshot) => published.push(snapshot));

  const changed = registry.refreshRenderedLines(
    "T",
    [themeCustom(["new-1"]), themeCustom(["new-5"]), themeCustom(["brand-new"])],
    ["e1", "e5", "e9"],
  );

  assert.equal(changed, true);
  assert.ok(published.length >= 1, "有替换就要发布快照（次数受异步管家影响，只看内容）");
  assert.equal(
    published[published.length - 1].messages[0].renderedLines[0],
    "new-1",
    "发布出去的快照已经是新色",
  );
  const after = registry.getSnapshot("T");
  assert.deepEqual(after.entryIds, ["e1", "e2", "e5", "e6"], "结构不变：不追加也不截断");
  assert.equal(after.messages[0].renderedLines[0], "new-1", "已 prepend 的更早页同样换色");
  assert.equal(after.messages[2].renderedLines[0], "new-5");
  assert.equal(after.messages.length, 4);
  assert.equal(after.timelineSeq, before.timelineSeq, "换色不递增 timelineSeq（否则并发 hydrate 会被判 stale 而丢消息）");
});

// 这条**不承重**：它锁的是「旧的 tail 归并会怎样」——说明本 issue 为什么不能用重拉一页来修。
// 修复被退回它仍然是绿的（那正是它证明的事）。承重的是上面那条「更早页也换新色」。
test("对照组（说明成因，不承重）：旧的 tail 归并在跳读历史时会把整个窗口换成尾页", () => {
  const stream = fakeStream();
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt(sessionId, input) {
      return { submissionId: input.submissionId, sessionId, status: "accepted" };
  ...
  // jumpToEntry 的 around 窗口：只含历史那几条
  registry.hydrate("T", [themeCustom(["old-10"]), themeCustom(["old-11"])], ["e10", "e11"], { mode: "replace" });
  // 「重拉一页」= tail：新尾页首条不在时间线里 → 整段换成尾页
  registry.hydrate("T", [themeCustom(["old-1"]), themeCustom(["old-2"])], ["e1", "e2"], { mode: "tail" });
  assert.deepEqual(
    registry.getSnapshot("T").entryIds,
    ["e1", "e2"],
    "tail 归并会替换掉 around 窗口（用户被从阅读位置拽回最新一屏）",
  );
  assert.equal(registry.getSnapshot("T").messages[0].renderedLines[0], "old-1", "换来的尾页本身仍是旧色");
});

test("主题刷新：换色后落地的**在途旧响应**不得把新色盖回去（反向竞态）", () => {
  const registry = themeRegistry();
  // 这次 hydrate 在换色**之前**发出（服务端当时还是旧主题），落地在换色之后。
  const inflight = registry.beginHydrate("T");
  registry.refreshRenderedLines("T", [themeCustom(["new-1"])], ["e1"]);
  assert.equal(registry.getSnapshot("T").messages[0].renderedLines[0], "new-1", "前置条件：换色已生效");

  registry.hydrate(
    "T",
    [themeCustom(["old-1"]), themeUser("u2"), themeCustom(["old-5"]), themeUser("u6"), themeUser("u7")],
    ["e1", "e2", "e5", "e6", "e7"],
    { mode: "tail", hydrateRequestSeq: inflight },
  );

  const after = registry.getSnapshot("T");
  assert.ok(after.entryIds.includes("e7"), "在途响应照常合并（换色不拦 hydrate）");
  assert.equal(after.messages[2].renderedLines[0], "old-5", "这次 hydrate 确实把磁盘页合并进来了（不是被跳过）");
  assert.equal(
    after.messages[0].renderedLines[0],
    "new-1",
    "先于换色发出的响应落地后，重叠段仍是新色（否则要等下一次整段 replace）",
  );
});

test("主题刷新：换色前发出、换色后落地且**内容已变**的响应用磁盘行（只比请求序号会把旧渲染盖回去）", () => {
  const registry = themeRegistry();
  const inflight = registry.beginHydrate("T");
  registry.refreshRenderedLines("T", [themeCustom(["new-1"])], ["e1"]);
  assert.equal(registry.getSnapshot("T").messages[0].renderedLines[0], "new-1", "前置条件：换色已生效");

  // 这份响应在换色**之前**发出（序号更小），但它带回来的 e1 并不是换色时替换掉的那批旧行 ——
  // 内容真的变了（context_edit / 插件渲染器输出不同）。此时必须让磁盘赢：
  // 只看「请求发出序号 < 换色位点」会把旧渲染盖在更新的内容上（第三轮审查指出的正是这条）。
  registry.hydrate(
    "T",
    [themeCustom(["edited-1"]), themeUser("u2"), themeCustom(["old-5"]), themeUser("u6")],
    ["e1", "e2", "e5", "e6"],
    { mode: "tail", hydrateRequestSeq: inflight },
  );
  ...
  assert.equal(
    after.messages[0].renderedLines[0],
    "edited-1",
    "内容变了就让磁盘赢（否则界面上显示的是过期渲染，而且要等下一次换色才纠正）",
  );
  assert.equal(after.messages[2].renderedLines[0], "old-5", "没被换色覆盖过的条目照常保留磁盘行");
});

test("主题刷新：换色**之后**发出的 hydrate 用磁盘行（记忆不覆盖更新内容）", () => {
  const registry = themeRegistry();
  registry.refreshRenderedLines("T", [themeCustom(["new-1"])], ["e1"]);
  // 这次读取是换色之后发出的：它拿到的就是当前主题的行（例如 context_edit 之后重渲染）。
  const later = registry.beginHydrate("T");
  registry.hydrate(
    "T",
    [themeCustom(["edited-1"]), themeUser("u2"), themeCustom(["old-5"]), themeUser("u6")],
    ["e1", "e2", "e5", "e6"],
    { mode: "tail", hydrateRequestSeq: later },
  );
  assert.equal(
    registry.getSnapshot("T").messages[0].renderedLines[0],
    "edited-1",
    "更新的磁盘行要赢，否则 context_edit 之后会一直看到旧渲染",
  );
});

test("主题刷新：会话没有 slot / 没有可替换项时不发布、返回 false", () => {
  const registry = themeRegistry();
  const beforeMessages = registry.getSnapshot("T").messages;
  assert.equal(registry.refreshRenderedLines("missing", [themeCustom(["x"])], ["e1"]), false, "没有 slot");
  assert.equal(registry.refreshRenderedLines("T", [themeCustom(["x"])], ["nope"]), false, "没有匹配项");
  assert.equal(
    registry.getSnapshot("T").messages,
    beforeMessages,
    "没有替换就不重建消息数组（不发布无谓快照）",
  );
});

test("主题刷新：期间有 live 落盘（timelineSeq 前进）也照样换色，不被当 stale 丢掉", () => {
  const registry = themeRegistry();
  // 先取出数字：snapshot 可能是同一对象被就地自增，持有引用会读到同一个值。
  const beforeSeq = registry.getSnapshot("T").timelineSeq;
  registry.applyEvent("T", {
    type: "message_end",
    entryId: "e7",
    message: { role: "user", content: "u7", timestamp: 7 },
  });
  assert.ok(
    registry.getSnapshot("T").timelineSeq > beforeSeq,
    "前置条件：live 落盘推进了 timelineSeq",
  );

  const changed = registry.refreshRenderedLines("T", [themeCustom(["new-1"])], ["e1"]);
  assert.equal(changed, true, "换色不读 sinceSeq：在途落盘不该把这次换色判成 stale");
  const after = registry.getSnapshot("T");
  assert.equal(after.messages[0].renderedLines[0], "new-1");
  assert.ok(after.entryIds.includes("e7"), "落盘的消息仍在（换色不缩短也不替换窗口）");
});
