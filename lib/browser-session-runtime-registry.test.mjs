...
test("吞吐读数：冷挂载用服务端 seed 兜底，本地跑完完整 step 后以本地为准", async () => {
  const stream = fakeStream();
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt() {
    ...
    async getAgentState() {
      return { live: true, activeRun: true, state: {} };
    },
    async ensureNewSession() {
      throw new Error("not used");
    },
  ...
  const sessionId = "metrics-seed-session";
  const subscription = registry.attach(sessionId);

  // 冷挂载（刷新/后台回收回来）：importRunningRun 只看到 step 尾部。
  registry.importRunningRun(sessionId);
  // 服务端读数（host 从 run 开始就在看）→ 页面直接显示，不必等本地跑完一个 step。
  registry.seedTurnMetrics(sessionId, { tokensPerSecond: 21, ttftMs: 380 });
  assert.deepEqual(registry.getSnapshot(sessionId).turnMetrics, { tokensPerSecond: 21, ttftMs: 380 });

  // 只观测到尾部（没有本 step 的 message_start）、却带整段 usage 的 step：
  // 不得算成虚高读数。
  stream.emit({ type: "message_update", message: { role: "assistant", content: [{ type: "text", text: "x" }] } });
  stream.emit({
    type: "message_end",
    message: { role: "assistant", content: [{ type: "text", text: "x" }], usage: { output: 5 } },
  });
  assert.deepEqual(
    registry.getSnapshot(sessionId).turnMetrics,
    { tokensPerSecond: 21, ttftMs: 380 },
    "部分观测的 step 不得产生虚高读数（应继续用服务端 seed）",
  );

  // 下一个 step 从头观测：本地读数生效并取代 seed。
  stream.emit({ type: "message_start", message: { role: "assistant", content: [{ type: "text", text: "y" }] } });
  await new Promise((resolve) => setTimeout(resolve, 20));
  stream.emit({
    type: "message_end",
    message: { role: "assistant", content: [{ type: "text", text: "y" }], usage: { output: 40 } },
  });
  const local = registry.getSnapshot(sessionId).turnMetrics;
  assert.equal(typeof local.tokensPerSecond, "number", "本地完整 step 后应有本地读数");
  assert.notEqual(local.tokensPerSecond, 21, "本地读数应取代服务端 seed");

  subscription.dispose();
  registry.resetForTests();
});

test("吞吐读数：没有 usage 的 step 不参与，也不清掉服务端 seed", async () => {
  const stream = fakeStream();
  const registry = createBrowserSessionRuntimeRegistry({
    async postPrompt() {
      throw new Error("not used");
  ...
  const sessionId = "metrics-no-usage-session";
  const subscription = registry.attach(sessionId);
  registry.importRunningRun(sessionId);
  registry.seedTurnMetrics(sessionId, { tokensPerSecond: 30 });
  stream.emit({ type: "message_start", message: { role: "assistant", content: [{ type: "text", text: "x" }] } });
  stream.emit({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: "x" }] } });
  assert.deepEqual(
    registry.getSnapshot(sessionId).turnMetrics,
    { tokensPerSecond: 30 },
    "无 usage 的 step 既不得产生本地读数，也不得清掉服务端 seed",
  );
  subscription.dispose();
  registry.resetForTests();
});
