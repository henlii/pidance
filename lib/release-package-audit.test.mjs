...
test("Windows 临时中性根位于 HOME 下时不误报", () => {
  if (process.platform !== "win32") return;
  const neutral = path.join(os.tmpdir(), "pidance-release-build");
  assert.deepEqual(
    scanSensitiveContent(JSON.stringify({ outputFileTracingRoot: neutral }), {
      sensitiveRoots: ["/home/dev/pidance-src"],
      neutralRoots: [neutral],
      homeDir: os.homedir(),
    }),
    [],
  );
