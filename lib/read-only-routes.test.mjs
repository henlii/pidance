...
test("state GET：readOnly 时不调用 getLive/ensureLive（源码契约 + stub）", async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      assert.deepEqual(await response.json(), { running: false, readOnly: true });
...
test("events GET：ensureLive 启动失败映射为 500 Failed to start", async () => {
  const originalIsReadOnly = sessionService.isReadOnly;
  const originalEnsureLive = sessionService.ensureLive;
  sessionService.isReadOnly = async () => false;
  sessionService.ensureLive = async () => {
  ...
  try {
    const response = await eventsGet(new Request("http://localhost/api/test"), {
      params: Promise.resolve({ id: "start-fail" }),
    });
    assert.equal(response.status, 500);
    const text = await response.text();
    assert.match(text, /Failed to start agent/);
    assert.match(text, /boom-start/);
  } finally {
    sessionService.isReadOnly = originalIsReadOnly;
    sessionService.ensureLive = originalEnsureLive;
  }
...
test("events GET：ensureLive Session not found → 404", async () => {
  const originalIsReadOnly = sessionService.isReadOnly;
  const originalEnsureLive = sessionService.ensureLive;
  sessionService.isReadOnly = async () => false;
  sessionService.ensureLive = async () => {
...
test("sessions GET/context 使用 getReadView；state 使用 getLive 不 ensureLive（源码）", () => {
  ...
  assert.match(sessionRoute, /getReadView/);
  assert.match(sessionRoute, /sessionService\.destroy/);
  ...
  assert.match(contextRoute, /getReadView/);
  assert.doesNotMatch(contextRoute, /getRpcSession|resolveSessionManagerForRead/);

  const stateRoute = readFileSync(`${root}app/api/sessions/[id]/state/route.ts`, "utf8");
  // 默认 getLive（不启动）；wake=1 时经 session-service.ensureLive（不直接 import rpc-manager）
  assert.match(stateRoute, /getLive/);
  ...
  assert.match(autoName, /ensureLive/);
  assert.doesNotMatch(autoName, /getRpcSession|startRpcSession/);

  const events = readFileSync(`${root}app/api/agent/[id]/events/route.ts`, "utf8");
  assert.match(events, /ensureLive/);
  assert.doesNotMatch(events, /getRpcSession|startRpcSession/);
});
