...
// ---------------------------------------------------------------------------
// Bug：allowedRoots 里有一个已消失的会话根（例如被删掉的项目/工作树目录）时，
// 保存整体失败并报「目标路径不存在」——用户实测：编辑 ~/.namixinxi/basis/.env
// 点保存就报这个，而读/建文件都正常（那两条路径本来就跳过失效根）。
// ---------------------------------------------------------------------------

test("失效的授权根不影响保存（跳过即可，与 file-read / file-ops 同口径）", async () => {
  const { saveFile } = await load(); const f = fixture();
  const missingRoot = path.join(os.tmpdir(), "pi-save-missing-root-does-not-exist");
  try {
    const s = fs.statSync(f.file);
    const result = save(saveFile, {
      target: f.file,
      cwd: f.root,
      allowedRoots: new Set([missingRoot, f.root]),
      content: "saved despite stale root",
      baseline: { mtimeMs: s.mtimeMs, size: s.size },
    });
    assert.equal(fs.readFileSync(f.file, "utf8"), "saved despite stale root");
    assert.equal(result.size, Buffer.byteLength("saved despite stale root"));
  } finally { f.clean(); }
});

test("会话 cwd 已消失（工作树被删）时保存仍可用", async () => {
  const { saveFile } = await load(); const f = fixture();
  const goneCwd = path.join(os.tmpdir(), "pi-save-gone-cwd-does-not-exist");
  try {
    const s = fs.statSync(f.file);
    save(saveFile, {
      target: f.file,
      cwd: goneCwd,
      allowedRoots: new Set([f.root]),
      content: "ok",
      baseline: { mtimeMs: s.mtimeMs, size: s.size },
    });
    assert.equal(fs.readFileSync(f.file, "utf8"), "ok");
  } finally { f.clean(); }
});
