...
const WINDOWS_FIXED_NEUTRAL_RELEASE_BUILD_ROOT = path.join(os.tmpdir(), "pidance-release-build");
...
export function isFixedNeutralReleaseBuildRoot(packageRoot, deps = {}) {
  ...
  const candidates = [resolvePath(FIXED_NEUTRAL_RELEASE_BUILD_ROOT)];
  if (process.platform === "win32") candidates.push(resolvePath(WINDOWS_FIXED_NEUTRAL_RELEASE_BUILD_ROOT));
  for (const fixed of candidates) {
    if (resolved !== fixed) continue;
    try {
      const real = realpathSync(resolved);
      const samePath = process.platform === "win32"
        ? normalizeSensitiveRoot(real).toLowerCase() === normalizeSensitiveRoot(fixed).toLowerCase()
        : real === fixed;
      if (samePath && statSync(resolved).isDirectory()) return true;
    } catch {
      // 继续检查其它固定候选根
    }
  }
...
export function resolveReleaseAuditRoots(input) {
  ...
  const fixed = resolvePath(
    process.platform === "win32" ? WINDOWS_FIXED_NEUTRAL_RELEASE_BUILD_ROOT : FIXED_NEUTRAL_RELEASE_BUILD_ROOT,
  );
  ...
  return {
    ...
    neutralRoot: normalizeSensitiveRoot(neutralReal),
...
export function scanSensitiveContent(text, opts = {}) {
  ...
  if (homeDir && homeDir.length >= 2 && homeDir !== "/") {
    ...
    if (!homeIsRoot) {
      ...
      // Windows 的固定候选构建根位于 HOME 下的 %TEMP%；只豁免这个精确根，
      // 仍保留对 HOME 下其它路径的扫描。
      let homeScanText = textNorm;
      for (const neutralRoot of opts.neutralRoots ?? []) {
        const normalized = normalizeSensitiveRoot(neutralRoot);
        if (!normalized) continue;
        const neutralEscaped = normalized.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        homeScanText = homeScanText.replace(new RegExp(`${neutralEscaped}(?=/|["'\\s]|$)`, "ig"), "__PIDANCE_NEUTRAL_ROOT__");
      }
      if (new RegExp(`${escaped}/`, "i").test(homeScanText)) {
...
export function auditReleasePackage(input) {
  ...
  for (const file of pack.files) {
    ...
    for (const reason of scanSensitiveContent(resolved.text, {
      ...
      neutralRoots: input.neutralRoots,
