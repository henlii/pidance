...
import { getRelativeFilePath, normalizeFilePathSlashes } from "./file-paths";
 ...
 *
 * 单独抽出来是因为这条规则有三面：@ 引用必须是相对路径（agent 的 read 工具按 cwd 解析），
 * 而卡片拿到的路径可能是绝对路径（apply_patch 的 appliedFiles）。不在 cwd 下的路径原样保留——
 * 宁可给一个绝对路径，也不要硬套一个错的相对路径。最后统一把路径分隔符归一成正斜杠：
 * `getRelativeFilePath` 的兜底返回原串，Windows 下会得到 `@D:\tmp\a.ts `，而 @ 引用与 read
 * 都按正斜杠解析（POSIX 路径不受影响，归一函数只处理盘符与 UNC 前缀）。
 */
export function buildFileReferenceText(filePath: string, cwd?: string): string {
  return buildAtMentionText(normalizeFilePathSlashes(getRelativeFilePath(filePath, cwd)), false);
}
