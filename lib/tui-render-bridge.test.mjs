...
// 渲染桥的主题实例是 SDK 的 `Theme` 类，由宿主静态 import 后注入（issue #97）；
// 生产路径是 lib/sdk-session-host.ts，测试从这里走同一条注入。
// 渲染桥会 import 同目录的纯模块（lib/kitty-image）：无扩展名相对 import 只有 jiti 解析得了
// —— 与仓库其它 lib 测试同一条约定。
const jiti = createJiti(import.meta.url, { tsconfigPaths: true });
const { setPiThemeConstructor, loadPiTheme } = await jiti.import("./tui-render-bridge.ts");
...
async function loadSubject() {
  return jiti.import("./tui-render-bridge.ts");
}
...
test("插件用未知颜色名 → 渲染桥返回 null 而不是抛出（issue #72 的兜底）", async () => {
  const { renderWidgetFactoryLines, renderCustomMessageLines, renderCustomEntryLines, loadPiTheme } = await loadSubject();
  const theme = loadPiTheme();
  assert.ok(theme, "主题副本应可加载");
  // 真 Theme.fg 对未知颜色会抛错；插件写错颜色名时不能让整条渲染打挂。
  assert.throws(() => theme.fg("not-a-color", "x"), /Unknown theme color/);
  assert.equal(renderWidgetFactoryLines(() => new Text(theme.fg("not-a-color", "boom")), theme), null);
  assert.equal(
    renderCustomMessageLines((_m, _o, th) => new Text(th.fg("not-a-color", "boom")), { role: "custom" }, theme),
    null,
  );
  assert.equal(
    renderCustomEntryLines((_e, _o, th) => new Text(th.fg("not-a-color", "boom")), { type: "custom" }, theme),
    null,
  );
});
