 ...
 * 工具调用），所以这里钉住源码契约；**行为**由 lib/tool-render-slots.test.mjs 覆盖
 * （真实 SdkSessionHost + 假工具定义，驱动 start/update/end 断言事件真的带渲染行、
 * result 之后就地把 diff 写回调用组件时要重读、推过的行不重复推）。
 */
...
test("宿主按 SDK 真实事件名接线：start 渲染 renderCall、end 渲染最终 renderResult", () => {
  ...
  const rereadAt = slotsBlock.indexOf("renderComponentLines(entry.lastCallComponent");
  assert.ok(rereadAt > resultAt, "result 之后必须重读调用组件（renderResult 会就地把 diff 写回它）");
});
