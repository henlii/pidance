...
test("buildSessionContext：压缩后仅保留可见 context 上的 activity（最小策略：压缩前 activity 不复活）", () => {
  ...
  const context = buildSessionContext(entries);
  // 可见：compaction + u2 + act-new + u3；act-old 在 firstKept 之前不展示
  assert.equal(context.entryIds.includes("act-old"), false);
  assert.ok(context.entryIds.includes("act-new"));
  assert.ok(context.messages.some((m) => m.content === "after-cmp"));
  assert.equal(context.messages.some((m) => m.content === "before-cmp"), false);
  assert.equal(context.messages.length, context.entryIds.length);
});
