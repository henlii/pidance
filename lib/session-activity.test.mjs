...
test("SessionService.appendActivity：in-process live（有 inner.sessionManager）走 wrapper.appendActivity", async () => {
  const calls = [];
  const live = {
    isAlive: () => true,
    ...
    destroy: () => {},
    // 仅有 inner.sessionManager 的 live 才视为 in-process 安全写路径
...
test("SessionService.appendActivity：外部 RPC live（无 inner）先 destroy 再写盘，不调 wrapper.appendActivity", async () => {
  const { mkdtempSync, rmSync, writeFileSync, readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const { tmpdir } = await import("node:os");
  const dir = mkdtempSync(join(tmpdir(), "act-sw-"));
  ...
  const live = {
    isAlive: () => destroyed === 0,
    // ExternalRpcSession 形态：有 appendActivity 但无 inner.sessionManager
    appendActivity: () => {
      appendCalled += 1;
      throw new Error("不得调用外部 live.appendActivity");
    },
    destroy: () => {
      destroyed += 1;
    },
  ...
  const service = createSessionService({
    getRpcSession: (id) => (id === "s-ext" && destroyed === 0 ? live : undefined),
  ...
  try {
    const result = await service.appendActivity("s-ext", {
      kind: "warning",
      title: "w",
      content: "body",
    });
    ...
    assert.match(text, /pidance\.activity/);
    assert.match(text, /"title":"w"/);
  } finally {
