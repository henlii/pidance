...
const { CHAT_COLUMN_WIDTH_DEFAULT_RATIO } = await jiti.import("./chat-column.ts");
...
test("偏好解析：含旧 collapsedWorktreePaths 的历史偏好仍可加载（该键被忽略且不再写回）", async () => {
  ...
  const prefs = parseSidebarPreferences({
    ...
    collapsedWorktreePaths: ["/repo-worktrees/feat"],
  });
  assert.deepEqual(prefs, DEFAULT_SIDEBAR_PREFERENCES);
  assert.equal(serializeSidebarPreferences(prefs).includes("collapsedWorktreePaths"), false);
});
...
test("偏好解析：alias 脏数据回退；旧模型（added/closed）迁移成单一 projectRoots", async () => {
  const { parseSidebarPreferences } = await jiti.import("./ui-preferences.ts");
  const legacy = parseSidebarPreferences({
    displayMode: "compact",
    projectAliases: { "/repo": "别名", "/bad": 1 },
    addedProjectRoots: ["/repo", "/closed", 2, null],
    closedProjectRoots: ["/closed"],
  });
  assert.deepEqual(legacy.projectAliases, { "/repo": "别名" });
  // 可见集合 = added − closed，且标记为「需要一次性种子」
  assert.deepEqual(legacy.projectRoots, ["/repo"]);
  assert.equal(legacy.projectRootsMigrated, true);
  // 新模型：直接用 projectRoots，不再迁移
  // 清洗：trim、去空、去重（保序）；不做绝对路径校验
  const current = parseSidebarPreferences({ projectRoots: ["/a", " /b ", "/a", "", null, 7] });
  assert.deepEqual(current.projectRoots, ["/a", "/b"]);
  assert.equal(current.projectRootsMigrated, false);
});
...
test("偏好解析：旧版本数据（无 alias/closed/sidebarWidth 字段）回退为空/默认宽", async () => {
  ...
  assert.deepEqual(prefs.projectRoots, []);
  // 完全没有项目相关字段的旧数据：不算迁移（没有可种子的来源）
  assert.equal(prefs.projectRootsMigrated, false);
...
test("无 window 环境：load 返回默认值、save 静默不抛", async () => {
  ...
  assert.doesNotThrow(() => saveSidebarPreferences({
    ...
    pinnedSessionIds: DEFAULT_SIDEBAR_PREFERENCES.pinnedSessionIds,
    ungroupedSessionIds: DEFAULT_SIDEBAR_PREFERENCES.ungroupedSessionIds,
...
test("applySyncedSidebarUi：远端还是旧模型且本地无列表 → 保持待迁移（种子仍会跑）", async () => {
  const {
    applySyncedSidebarUi,
    DEFAULT_SIDEBAR_PREFERENCES,
  } = await jiti.import("./ui-preferences.ts");
  // 新设备（无 localStorage）+ 服务端旧模型载荷：必须继续待迁移，否则项目区空着
  const legacyRemote = { displayMode: "standard", addedProjectRoots: [], closedProjectRoots: [] };
  const pending = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, legacyRemote);
  assert.equal(pending.projectRootsMigrated, true);
  // 本地已有项目列表（旧模型迁移出来的或用户已加过）：不再种子
  const withList = applySyncedSidebarUi({ ...DEFAULT_SIDEBAR_PREFERENCES, projectRoots: ["/a"] }, legacyRemote);
  assert.equal(withList.projectRootsMigrated, false);
  assert.deepEqual(withList.projectRoots, ["/a"]);
  // 远端已经是新模型：迁移结束，采用远端列表
  const current = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, { projectRoots: ["/b"] });
  assert.equal(current.projectRootsMigrated, false);
  assert.deepEqual(current.projectRoots, ["/b"]);
});

test("#55 迁移计划：只迁 added − closed，无旧键不种任何东西", async () => {
  const { planLegacyProjectRootsMigration } = await jiti.import("./ui-preferences.ts");
  // 有旧键：可见集合 = added − closed（trim/去空/去重由 sanitize 保证）
  assert.deepEqual(
    planLegacyProjectRootsMigration({ added: ["/a", " /b ", "/closed", 2, null], closed: ["/closed"], hasProjectRoots: false }),
    { projectRoots: ["/a", "/b"] },
  );
  // 只有 closed（没有 added）：可见集合为空，仍然是一次「迁移」，不是种子
  assert.deepEqual(planLegacyProjectRootsMigration({ added: undefined, closed: ["/x"], hasProjectRoots: false }), { projectRoots: [] });
  // 完全没有旧键：不种任何东西（这正是被撑大列表的那条路径）
  assert.equal(planLegacyProjectRootsMigration({ added: undefined, closed: undefined, hasProjectRoots: false }), null);
  // 已是新模型：不迁移
  assert.equal(planLegacyProjectRootsMigration({ added: ["/a"], closed: [], hasProjectRoots: true }), null);
});

test("#55 applySyncedSidebarUi：远端缺少项目字段不再被当成「待迁移」", async () => {
  const { applySyncedSidebarUi, DEFAULT_SIDEBAR_PREFERENCES } = await jiti.import("./ui-preferences.ts");
  // 远端载荷里既没有 projectRoots 也没有旧键（例如只带了 displayMode）：不该重新打开迁移
  const noProjectKeys = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, { displayMode: "compact", projectSort: "az" });
  assert.equal(noProjectKeys.projectRootsMigrated, false, "没有旧键就不该待迁移（否则种子会把会话 cwd 并进列表）");
  assert.deepEqual(noProjectKeys.projectRoots, []);
  // 远端确实带旧键（服务端旧模型）：仍然按旧模型迁移
  const legacyRemote = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, {
    addedProjectRoots: ["/repo", "/closed"],
    closedProjectRoots: ["/closed"],
  });
  assert.equal(legacyRemote.projectRootsMigrated, true);
  assert.deepEqual(legacyRemote.projectRoots, ["/repo"]);
  // 本地已有列表：不被远端旧键覆盖，且不再迁移
  const local = applySyncedSidebarUi({ ...DEFAULT_SIDEBAR_PREFERENCES, projectRoots: ["/mine"] }, {
    addedProjectRoots: ["/repo"],
    closedProjectRoots: [],
  });
  assert.equal(local.projectRootsMigrated, false);
  assert.deepEqual(local.projectRoots, ["/mine"]);
});

test("#55 旧客户端 + 服务端已有列表：采用远端列表、不再迁移覆盖（历史上被撑大的那条路径）", async () => {
  const { parseSidebarPreferences, applySyncedSidebarUi } = await jiti.import("./ui-preferences.ts");
  // localStorage 是旧模型（added/closed，没有 list）
  const localLegacy = parseSidebarPreferences({ displayMode: "standard", addedProjectRoots: [], closedProjectRoots: [] });
  assert.equal(localLegacy.projectRootsMigrated, true);
  assert.deepEqual(localLegacy.projectRoots, []);
  // 服务端已是新模型：采用远端列表，迁移标记清掉 → 侧栏不会再跑种子、也不会用本地旧列表覆盖
  const synced = applySyncedSidebarUi(localLegacy, { projectRoots: ["/repo", "/other"] });
  assert.deepEqual(synced.projectRoots, ["/repo", "/other"]);
  assert.equal(synced.projectRootsMigrated, false);
});

test("#55 服务端仍是旧模型：迁移只取旧键可见集合，且不碰会话 cwd", async () => {
  const { parseSidebarPreferences, applySyncedSidebarUi, DEFAULT_SIDEBAR_PREFERENCES } = await jiti.import("./ui-preferences.ts");
  // 全新浏览器（无 localStorage）+ 服务端旧模型：种子只来自 added − closed
  const synced = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, {
    addedProjectRoots: ["/added"],
    closedProjectRoots: ["/added", "/hidden"],
  });
  assert.deepEqual(synced.projectRoots, [], "added 里被 closed 的那条不算可见集合");
  assert.equal(synced.projectRootsMigrated, true);
  // 没有旧键的旧模型载荷：不迁移（原先会按「本地列表为空」重开迁移，才会把会话 cwd 并进来）
  const noKeys = applySyncedSidebarUi(DEFAULT_SIDEBAR_PREFERENCES, { displayMode: "standard" });
  assert.equal(noKeys.projectRootsMigrated, false);
  assert.deepEqual(noKeys.projectRoots, []);
});

test("#53 未分组标记：逐项过滤脏数据、去重保序，序列化往返保持，超限保留最后写入的部分", async () => {
  const { parseSidebarPreferences, serializeSidebarPreferences, UNGROUPED_SESSION_IDS_LIMIT } = await jiti.import("./ui-preferences.ts");
  const prefs = parseSidebarPreferences({ ungroupedSessionIds: ["a", " b ", 1, null, "", "a", {}, "c"] });
  assert.deepEqual(prefs.ungroupedSessionIds, ["a", "b", "c"]);
  // 旧数据缺字段 / 脏类型 → 空集合；绝不抛异常，也不回退成其它字段的值
  assert.deepEqual(parseSidebarPreferences({}).ungroupedSessionIds, []);
  assert.deepEqual(parseSidebarPreferences({ ungroupedSessionIds: "x" }).ungroupedSessionIds, []);
  assert.deepEqual(parseSidebarPreferences({ ungroupedSessionIds: { a: 1 } }).ungroupedSessionIds, []);
  // 序列化往返：集合原样保留
  assert.deepEqual(
    parseSidebarPreferences(JSON.parse(serializeSidebarPreferences(prefs))).ungroupedSessionIds,
    ["a", "b", "c"],
  );
  // 上限兜住无界增长（本期只增不减）：超限丢弃最早写入的标记
  const many = Array.from({ length: UNGROUPED_SESSION_IDS_LIMIT + 3 }, (_, i) => `s${i}`);
  const capped = parseSidebarPreferences({ ungroupedSessionIds: many }).ungroupedSessionIds;
  assert.equal(capped.length, UNGROUPED_SESSION_IDS_LIMIT);
  assert.equal(capped[0], "s3");
  assert.equal(capped[capped.length - 1], `s${UNGROUPED_SESSION_IDS_LIMIT + 2}`);
});

test("#53 未分组标记随跨端同步载荷一起走；旧客户端缺该字段时保留本地值", async () => {
  const { parseSidebarPreferences, sidebarUiFromPrefs, applySyncedSidebarUi } = await jiti.import("./ui-preferences.ts");
  const prefs = parseSidebarPreferences({ ungroupedSessionIds: ["a"] });
  assert.deepEqual(sidebarUiFromPrefs(prefs).ungroupedSessionIds, ["a"]);
  assert.deepEqual(applySyncedSidebarUi(prefs, { ungroupedSessionIds: ["a", "b"] }).ungroupedSessionIds, ["a", "b"]);
  assert.deepEqual(applySyncedSidebarUi(prefs, { displayMode: "compact" }).ungroupedSessionIds, ["a"]);
});
