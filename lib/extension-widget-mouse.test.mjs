...
test("像素→单元格：与 render 同源 —— 换算出的列数就是上报宽度能放的字符数", () => {
  // 服务端按 renderWidth=100 列渲染；前端这个 pre 的字符宽 8px 时，正文区宽度
  // 必须是 100*8 + padding，换算出的 width 才会正好是 100。
  const point = widgetCellFromMetrics(metrics({ bodyWidth: 800 }));
  assert.equal(point?.width, 100);
});
...
test("滚动：手指位移超过阈值就取消长按，也不再算 tap", () => {
  const press = touchStart();
  ...
  const ended = touchEnd(scrolled);
  assert.equal(consumeTapClick(ended).send, true, "浏览器滑动后本来也不发 click；真发了也只是一次普通点击");
});
