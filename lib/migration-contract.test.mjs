...
test("ExternalRpcSession 发出 prompt_done 与 agent_end 后清 sidecar", async () => {
  const src = await read("lib/pi-runtime/external-session.ts");
  assert.match(src, /case "agent_end"/);
  assert.match(src, /type: "prompt_done"/);
  assert.match(src, /clearLeafSidecar\(this\.realSessionFile\)/);
});

test("agent 命令清单：前端发送的 type 在 capabilities 或 external send 中有落点", async () => {
  const external = await read("lib/pi-runtime/external-session.ts");
  const caps = await read("lib/pi-runtime/rpc-capabilities.ts");
  const combined = `${external}\n${caps}`;
  for (const type of REQUIRED_AGENT_COMMANDS) {
    // set_thinking_level 等可能只在 capabilities 表；fork 等在 case 分支
    assert.ok(
      combined.includes(`"${type}"`) || combined.includes(`'${type}'`),
      `命令 ${type} 应在 external-session 或 capabilities 中出现`,
    );
  }
});
