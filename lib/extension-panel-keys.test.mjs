...
test("归属判据：插件面板自己的 keytrap 算「DOM 归属」，窗口 ③ 不许抢", () => {
  // 面板打开时（默认 focus: 'panel'）会把焦点放到自己的 textarea keytrap 上，而 keytrap
  // 自己有按键通路：转成终端序列后经 extension_ui_input 送进面板组件的 handleInput。
  // 若这里判成「不归 DOM」，窗口 ③ 会在捕获阶段把键截走、只送给 onTerminalInput，
  // 面板组件收不到键 —— 用户看到的就是「面板能显示，方向键/回车全无反应」（真实回归）。
  const keytrap = {
    ownerDocument: fakeDom().doc,
    closest: (selector) => (selector === EXTENSION_KEYTRAP_SELECTOR ? keytrap : null),
    contains: (other) => other === keytrap,
  };
  assert.equal(isDomOwnedKeyTarget(keytrap, keytrap), true, "keytrap 聚焦时窗口 ③ 必须让开");
  assert.equal(isDomOwnedKeyTarget(keytrap, null), true, "事件目标是 keytrap 就让开");
  assert.equal(isDomOwnedKeyTarget(null, keytrap), true, "焦点在 keytrap 也让开");
  // 对照组：没有面板元素参与时，窗口 ③ 照常接管（否则子代理部件的按键会失效）。
  const plain = focusable();
  assert.equal(plain.closest(EXTENSION_KEYTRAP_SELECTOR), null, "对照组元素不在面板里");
  assert.equal(isDomOwnedKeyTarget(plain, plain), true, "壳控件仍必须保住按键");
});
