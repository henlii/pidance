...
test("CSRF 不依赖 SameSite：带会话 Cookie 的跨站 POST 仍被拒（issue #86）", () => {
	// Cookie 是否随请求发出由浏览器的 SameSite 决定，服务端不能靠它做防护。
	// 这里带着会话 Cookie 头证明：即使带着凭据，跨站 POST 仍被 checkCsrf 挡下
	// （SameSite 从 Strict 改成 Lax 之后依旧）。
	const cookie = `${UI_SESSION_COOKIE_NAME}=dummy-session-value`;
	assert.equal(
		guardRequest(h({ method: "POST", cookie, secFetchSite: "cross-site", origin: "http://evil.example" }), EMPTY_ENV),
		"csrf",
	);
	// 同源 POST 带同一个 Cookie 不因 Lax 被误判
	assert.equal(
		guardRequest(h({ method: "POST", cookie, secFetchSite: "same-origin", origin: "http://127.0.0.1:31415" }), EMPTY_ENV),
		"ok",
	);
});
