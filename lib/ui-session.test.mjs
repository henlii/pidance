...
test("TTL：trustDevice 长期有效（10 年），默认 12h", async () => {
  ...
  // 信任设备 10 年（长期有效，删除才失效）
  assert.equal(UI_TRUSTED_DEVICE_TTL_MS, 10 * 365 * 24 * 60 * 60 * 1000);
});
...
test("Cookie 解析与 Set-Cookie 头", async () => {
  ...
  // SameSite=Lax（issue #86）：手机端从其它 App／通知顶层导航进来时 Strict 不带
  // Cookie 会掉登录；跨站 POST 的防护由 middleware 的 checkCsrf 负责，不靠 SameSite。
  assert.match(header, /SameSite=Lax/);
  assert.doesNotMatch(header, /SameSite=Strict/);
...
test("设备注册表：save / read / remove / has / 过期过滤 / 损坏降级", async (t) => {
  ...
  // 临时目录必须随测试回收：漏了会按跑测试的次数累积在 /tmp（曾达到 800+ 个）。
  ...
  // 信任设备（10 年 TTL）长期有效：远超 7 天仍有效，删除才失效
  const trusted = { id: "d3", label: "Chrome · Windows", createdAt: now, expiresAt: now + 10 * 365 * 24 * 60 * 60 * 1000 };
  saveUiSessionDevice(trusted, file, undefined, now);
  assert.equal(hasUiSessionDevice("d3", file, undefined, now + 8 * 24 * 60 * 60 * 1000), true);
  assert.equal(hasUiSessionDevice("d3", file, undefined, now + 365 * 24 * 60 * 60 * 1000), true);
  removeUiSessionDevice("d3", file, undefined, now);
  assert.equal(hasUiSessionDevice("d3", file, undefined, now), false);
  // 删除
...
test("isUiSessionActive：验签 + 设备注册校验；删除设备后失效", async (t) => {
