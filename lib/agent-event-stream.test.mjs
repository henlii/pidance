...
test("message_update 保留思考正文，供思考块流式展示", () => {
  const event = {
    type: "message_update",
    assistantMessageEvent: { big: true },
    message: {
      role: "assistant",
      content: [
        { type: "thinking", thinking: "流式思考", thinkingSignature: "reasoning_content" },
        { type: "text", text: "回答" },
      ],
    },
  };
  const result = projectAgentEvent(event);
  assert.equal(result.message.content[0].thinking, "流式思考");
  assert.equal(result.message.content[1].text, "回答");
  assert.ok(!("assistantMessageEvent" in result));
});
...
test("agent_end 瘦身为边界并保留 host contextUsage 快照", () => {
	assert.deepEqual(
		projectAgentEvent({ type: "agent_end", reason: "done", stats: { tokens: 1 } }),
		{ type: "agent_end" },
	);
	const usage = { contextWindow: 128_000, percent: 12.5, tokens: 16_000 };
	assert.deepEqual(
		projectAgentEvent({ type: "agent_end", contextUsage: usage }),
		{ type: "agent_end", contextUsage: usage },
	);
	// run 序号（复核 G1）：客户端靠它丢弃迟到的上一轮终止事件，必须随瘦身一起保留。
	assert.deepEqual(
		projectAgentEvent({ type: "agent_end", streamRunSeq: 7, stats: { tokens: 1 } }),
		{ type: "agent_end", streamRunSeq: 7 },
	);
});

test("message_end 透传 host contextUsage 快照（run 期间随助手消息更新）", () => {
	const usage = { contextWindow: 128_000, percent: 12.5, tokens: 16_000 };
	const event = { type: "message_end", message: { role: "assistant", content: [] }, contextUsage: usage };
	const result = projectAgentEvent(event);
	assert.equal(result, event, "message_end 原样透传（不复制大消息）");
