...
test("message_update 剥离流式 thinking 内容（保留块结构与签名，不改原对象）", () => {
	const event = {
		type: "message_update",
		message: {
			role: "assistant",
	...
	assert.equal(blocks[0].thinking, "");
	assert.equal(blocks[0].thinkingSignature, "reasoning_content");
	assert.equal(blocks[1].text, "回答");
	// 原对象不被修改
	assert.equal(event.message.content[0].thinking, "完整思考内容");
});
