...
const { openSessionView, createSessionManager, materializeSessionFile } = await _jitiSession.import("./pi-session-io.ts");
...
test("写入临时 jsonl 后可被 openSessionView 再打开", async () => {
  ...
  try {
    ...
    const reA = openSessionView(outA);
    const reB = openSessionView(outB);
    assert.deepEqual(
    ...
    // 纯函数路径与文件路径一致
    const pure = buildSessionBranchJsonl(openSessionView(sessionFile), {
      leafId: leafA,
