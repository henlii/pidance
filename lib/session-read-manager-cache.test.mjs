...
import { appendFileSync, mkdtempSync, rmSync, statSync, unlinkSync, utimesSync, writeFileSync } from "node:fs";
...
const { getSessionEntries, resolveSessionManagerForRead } = await jiti.import("./session-reader.ts");
...
test("sidecar 同字节数改写：按 leaf id 失效，不返回旧 leaf", () => {
  invalidateSessionReadCache();
  const { dir, path } = tempSession("session-e", { id: "session-e", messages: 3 });
  const sidecar = `${path}.leaf.json`;
  try {
    writeFileSync(sidecar, JSON.stringify({ version: 1, leafId: "e0" }, null, 2));
    const before = openCachedSessionReadView(path);
    assert.equal(before.getLeafId(), "e0");

    // 同长度改写并把时间戳钉回原值：只看 size/mtime 的指纹看不出任何变化
    const stamp = statSync(sidecar);
    writeFileSync(sidecar, JSON.stringify({ version: 1, leafId: "e1" }, null, 2));
    utimesSync(sidecar, stamp.atime, stamp.mtime);
    assert.equal(statSync(sidecar).size, stamp.size, "测试前提：字节数相同");

    const after = openCachedSessionReadView(path);
    assert.notEqual(after, before, "leaf 变了就必须重新解析");
    assert.equal(after.getLeafId(), "e1");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
/**
 * 授权路径的护栏（issue #109 顺带补的）：`getSessionEntries` 决定 /api/files 的文件授权，
 * **绝不能**经过这份按指纹复用的只读缓存。
 *
 * 手法：先让缓存里真的存下这份会话（真实条目、指纹正确），再把缓存里那个视图对象的
 * `getEntries` 换成毒药，然后调 `getSessionEntries`。只要它走缓存就会拿到毒药
 * （条数 0）而转红；走的是自己的 `openSessionView` 才会拿到真实的 2 条。
 */
test("getSessionEntries 不经过只读缓存（授权路径不能被缓存污染）", () => {
  invalidateSessionReadCache();
  const { dir, path } = tempSession("session-h", { id: "session-h", messages: 2 });
  try {
    const cached = openCachedSessionReadView(path);
    assert.equal(sessionReadCacheStats().entries, 1, "前提：缓存里已有这份会话");

    let poisonedCalls = 0;
    const realGetEntries = cached.getEntries.bind(cached);
    cached.getEntries = () => {
      poisonedCalls += 1;
      return [];
    };
    try {
      ...
      assert.equal(poisonedCalls, 0, "授权路径不得触碰缓存视图");
      assert.equal(entries.length, 2, "必须拿到磁盘上的真实 entries");
    } finally {
      cached.getEntries = realGetEntries;
    }

    // 对照：读视图路径**在**缓存命中时确实复用同一份（说明上面那个毒药是有效的）
    cached.getEntries = () => {
      poisonedCalls += 1;
      return [];
    };
    try {
      const viaCache = resolveSessionManagerForRead({ filePath: path });
      assert.equal(viaCache.getEntries().length, 0, "对照组：缓存路径应当拿到毒药，证明毒药可观测");
