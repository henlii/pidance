...
test("visibleCrumbEntries: 宽屏全留；窄屏超过两段时只留首尾中间插 gap", () => {
  const crumbs = [session("root"), session("mid"), session("now")];
  assert.deepEqual(visibleCrumbEntries(crumbs, { compact: false }), crumbs);
  // 窄屏但只有两段（根 + 当前）→ 都留，不用省略号占位
  assert.deepEqual(visibleCrumbEntries(crumbs.slice(1), { compact: true }), crumbs.slice(1));
  assert.deepEqual(visibleCrumbEntries([crumbs[0]], { compact: true }), [crumbs[0]]);
  assert.deepEqual(visibleCrumbEntries(crumbs, { compact: true }), [crumbs[0], "gap", crumbs[2]]);
});
