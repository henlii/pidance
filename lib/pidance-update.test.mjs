...
function isNpmCmd(cmd) {
  const base = String(cmd).replaceAll("\\", "/").split("/").pop();
...
test("applyPidanceUpdate：正式布局安装并切换 current（不重启）", async () => {
  ...
  try {
    ...
    assert.equal(calls.some((c) => isNpmCmd(c.cmd)), true);
    // current 指向新 release
...
test("applyPidanceUpdate：重启正式服务前已清理旧 release", async () => {
  const root = mkdtempSync(join(tmpdir(), "pidance-apply-restart-"));
  try {
    const oldRel = join(root, "releases", "0.1.0");
    const staleRel = join(root, "releases", "0.0.9");
    const oldPkg = join(oldRel, "node_modules", "@henlii", "pidance");
    ...
    const execFileImpl = async (cmd, args, opts) => {
      ...
      if (cmd === "systemctl" && args[0] === "restart") {
        releasesAtRestart = readdirSync(join(root, "releases")).sort();
        return { stdout: "", stderr: "" };
      }
      throw new Error(`unexpected ${cmd} ${args.join(" ")}`);
    };

    const result = await m.applyPidanceUpdate({
      cwd: oldPkg,
      env: {},
      fetchImpl,
      execFileImpl,
    ...
    assert.deepEqual(releasesAtRestart, ["0.1.0", "0.1.1"]);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("interpretServiceRestart：命令成功或随后 active 都算重启成功", () => {
  assert.deepEqual(m.interpretServiceRestart({ restartOk: true, laterActive: false }), { restarted: true });
  assert.deepEqual(
    m.interpretServiceRestart({ restartOk: false, restartError: "timeout", laterActive: true }),
    { restarted: true },
  );
  const failed = m.interpretServiceRestart({
    restartOk: false,
    restartError: "timeout of 120000ms exceeded",
    laterActive: false,
  });
  assert.equal(failed.restarted, false);
  assert.match(failed.message, /timeout of 120000ms exceeded/);
});

test("isSystemdUnitActive：exit 0 或 stdout=active", () => {
  assert.equal(m.isSystemdUnitActive("inactive\n", 3), false);
  assert.equal(m.isSystemdUnitActive("active\n", 0), true);
  assert.equal(m.isSystemdUnitActive("active\n", 3), true);
  assert.equal(m.isSystemdUnitActive("deactivating\n", 3), false);
});

test("restartOfficialPidanceService：restart 超时后 is-active 成功则不算失败", async () => {
  const calls = [];
  ...
  const r = await m.restartOfficialPidanceService({
    execFileImpl,
    env: {},
  ...
  assert.equal(calls[0], "systemctl restart pidance.service");
  assert.ok(calls.some((c) => c === "systemctl is-active pidance.service"));
});

test("restartOfficialPidanceService：始终 inactive 才报失败", async () => {
  let now = 0;
  const execFileImpl = async (_cmd, args) => {
    if (args[0] === "restart") throw new Error("timeout of 120000ms exceeded");
  ...
  assert.match(r.message, /timeout of 120000ms exceeded/);
});
...
test("applyPidanceUpdate：npm 走 resolveNpmBin 而非 PATH 里的 npm", async () => {
  const root = mkdtempSync(join(tmpdir(), "pidance-apply-npmbin-"));
  const npmDir = mkdtempSync(join(tmpdir(), "pidance-fake-npm-"));
  try {
    ...
    globalThis.__piPidanceLatestCache = undefined;
    const cmds = [];
    const execFileImpl = async (cmd, _args, opts) => {
      cmds.push(cmd);
      if (isNpmCmd(cmd)) {
        const pkgDir = join(opts.cwd, "node_modules", "@henlii", "pidance");
        mkdirSync(pkgDir, { recursive: true });
    ...
    const r = await m.applyPidanceUpdate({
      cwd: oldPkg,
      env: { PIDANCE_SKIP_SERVICE_RESTART: "1", npm_execpath: fakeNpm },
      fetchImpl: async () => new Response(JSON.stringify({ version: "0.1.1" }), { status: 200 }),
      execFileImpl,
      restartService: false,
    });
    assert.equal(r.ok, true);
    assert.equal(cmds[0], fakeNpm);
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("restartOfficialPidanceService：用户级 unit 走 systemctl --user", async () => {
  const calls = [];
  const execFileImpl = async (cmd, args) => {
    calls.push(`${cmd} ${args.join(" ")}`);
  ...
  assert.equal(r.restarted, true);
  assert.equal(calls[0], "systemctl --user is-active pidance.service");
  assert.ok(calls.some((c) => c === "systemctl --user restart pidance.service"));
});
