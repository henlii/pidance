...
test("#42 图片 outbox：文件缺失时 fail-closed（不投递、不丢文本、条目留在队列）", async () => {
  await withHost(async ({ host, hold, persistedQueue }) => {
    hold();
    const receipt = await host.send({
      type: "set_follow_up_queue",
    ...
    assert.equal(delivered, undefined, "缺图时不得调用 SDK（不得把正文单独发出去）");
    assert.deepEqual(
      persistedQueue().map((item) => [item.text, item.state]),
      [["图片丢了也要发", "waiting"]],
      "载荷必须留在队列（正文与引用都在）",
...
test("#42 自动投递：缺图时停在队列，不静默发纯文本", async () => {
  await withHost(async ({ host, hold, persistedQueue }) => {
    ...
    const receipt = await host.send({
      type: "set_follow_up_queue",
      items: [{ text: "自动投递的图", images: [dataImage()] }],
    });
    rmSync(receipt.items[0].images[0].path);
    const originalSend = host.send.bind(host);
    const prompts = [];
    ...
    // 直接跑自动投递的那一步（同一函数）：缺图必须停在队列，而不是发纯文本。
...
test("#42 派发时不得把 unknown 条目（及其图片）一并删掉", async () => {
  await withHost(async ({ host, hold, agentDir }) => {
    hold();
    const receipt = await host.send({
      type: "set_follow_up_queue",
      items: [
        { text: "结果未知的消息", images: [dataImage()] },
        { text: "这批要派发" },
      ],
    });
    const unknownPath = receipt.items[0].images[0].path;
    ...
    // 重启恢复路径：重新从 prefs 水合（claimed 只能变成 unknown）。
    ...
    assert.equal(dispatched.ok, true);
    assert.equal(delivered?.message, "这批要派发", "只投递 waiting 的那批");
    ...
    assert.equal(existsSync(unknownPath), true, "unknown 条目的图片不得被清扫");
  });
});
...
test("#42 纯图条目：可入队、可恢复、可投递", async () => {
  await withHost(async ({ host, hold }) => {
    ...
    assert.deepEqual(delivered?.images, [{ type: "image", data: PNG_1X1, mimeType: "image/png" }]);
    assert.equal(delivered?.message, "", "纯图消息就是空正文 + 图片");
  });
});

test("#42 第二张图落盘失败：本次已写出的图立即回收（不留孤儿文件）", async () => {
  await withHost(async ({ host, hold, agentDir }) => {
    hold();
    const outbox = join(agentDir, "pidance-attachments", "queue-outbox", host.sessionId);
    const receipt = await host.send({
      type: "set_follow_up_queue",
      items: [
        {
          ...
          images: [
            dataImage(),
            // 第二张是引用，但目标文件不存在：解析阶段合法，落盘阶段失败。
            { source: "ref", id: "ghost.png", path: `${outbox}/ghost.png`, mimeType: "image/png", name: "ghost.png" },
          ],
        },
      ],
    });
    assert.equal(receipt.ok, false, "有一张存不住就不能声称已入队");
    assert.equal(receipt.persist, true);
    assert.deepEqual(readdirSync(outbox), [], "第一张已写出的图必须回滚，不得留下孤儿文件");
  });
});
