...
test("#42 整包写入幂等：同一 submissionId 的重发只能拿到第一次的结果（I1）", async () => {
  await withHost(async ({ host, hold, persistedQueue }) => {
    hold();
    ...
    assert.deepEqual(first.items.map((item) => item.text), ["once"]);

    // 客户端没收到回执而重试同一次写入：revision 已经前进，但不得作为新写入再生效。
    ...
    assert.deepEqual(retry, first, "同 id 必须返回第一次的回执");
    assert.deepEqual(host.followUpQueue.map((item) => item.text), ["once"]);
    assert.deepEqual(persistedQueue().map((item) => item.text), ["once"], "磁盘上也只有第一次的内容");
  });
});

test("#42 整包写入幂等：冲突回执同样按 submissionId 缓存（I1）", async () => {
  await withHost(async ({ host, hold }) => {
    hold();
    await host.send({ type: "set_follow_up_queue", items: ["first"], submissionId: "seed" });
    const conflict = await host.send({
      type: "set_follow_up_queue",
      items: ["stale"],
      submissionId: "conflict-set",
      expectedRevision: 9_999,
    });
    assert.equal(conflict.conflict, true);

    // 重发同一次提交，这次带的是正确 revision：已判定过的提交不得再应用。
    const retry = await host.send({
      type: "set_follow_up_queue",
      items: ["stale"],
      submissionId: "conflict-set",
    ...
    assert.equal(retry.conflict, true, "同 id 返回第一次的冲突回执");
    assert.deepEqual(host.followUpQueue.map((item) => item.text), ["first"]);
  });
});
