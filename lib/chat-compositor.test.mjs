...
test("引导乐观 user 后置到 live 之后（disk0 → live → disk1）", async () => {
  const { composeChatPlan, getChatPlanLiveMessage } = await load();
  const steerUser = { ...user("steer"), _steerOptimistic: true };
...
test("无 trailing user 时 live 仍在末尾；live 后有 user 时仍能提取 live", async () => {
  ...
  const steerPlan = compose(composeChatPlan, [user("q"), { ...user("steer"), _steerOptimistic: true }], {
    isStreaming: true,
    agentOrBashRunning: true,
    liveSlot: { message: assistant([text("live")]), isActive: true },
  });
...
test("收尾把末轮收成过程组后，收窗口才能保住原 startIndex", async () => {
  const { composeChatPlan } = await load();
  const { shrinkVisibleCountOnPlanShrink, getVisibleRenderWindow } = await import("./chat-lazy-load.ts");
  const messages = [];
  for (let i = 0; i < 40; i++) {
    messages.push(user(`q${i}`), assistant([text(`a${i}`)]));
  }
  messages.push(user("last"));
  for (let i = 0; i < 20; i++) {
    messages.push(assistant([{ type: "thinking", thinking: `t${i}` }, tool(`c${i}`)]));
  }
  messages.push(assistant([text("final")]));
  const running = compose(composeChatPlan, messages, { agentOrBashRunning: true });
  const idle = compose(composeChatPlan, messages, { agentOrBashRunning: false });
  assert.ok(running.length > idle.length, `收尾分组应缩短计划 ${running.length}→${idle.length}`);
  const visible = 80;
  const before = getVisibleRenderWindow(running.length, visible);
  const unfixed = getVisibleRenderWindow(idle.length, visible);
  const shrunk = shrinkVisibleCountOnPlanShrink(visible, running.length, idle.length);
  const fixed = getVisibleRenderWindow(idle.length, shrunk);
  assert.ok(unfixed.startIndex < before.startIndex, "不收窗口会把更早内容挂进窗口");
  assert.equal(fixed.startIndex, before.startIndex);
});
