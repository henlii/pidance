...
test("stripLabelMetadataNodes：超长链迭代处理不栈溢出，label 提升语义不变", async () => {
  const { stripLabelMetadataNodes } = await jiti.import("./session-reader.ts");
  ...
  const result = stripLabelMetadataNodes([node]);
  ...
  const hoisted = stripLabelMetadataNodes(labelTree);
