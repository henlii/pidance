...
test("读盘喂给渲染器的必须是消息形状，与实时路径同一形状（issue #76 修复轮）", () => {
  const entries = [userEntry("u1", null, "start"), customMessageEntry("c1", "u1", "subagent-notify", "raw text")];
  const seen = [];
  buildSessionContext(entries, undefined, {
    messageLines: (message) => {
      seen.push(message);
      return ["line"];
    },
  });
  // 实时路径是 SDK 的 `createCustomMessage`：{role:"custom", customType, content, display, details, timestamp:number}。
  // JSONL entry 没有 role、timestamp 是 ISO 字符串——直接喂过去，只读这两个字段的渲染器会拿到 undefined。
  assert.equal(seen[0].role, "custom", "渲染器必须拿到 role=custom");
  assert.equal(typeof seen[0].timestamp, "number", "timestamp 必须是数字（不是 ISO 字符串）");
  assert.deepEqual(
    Object.keys(seen[0]).sort(),
    ["content", "customType", "details", "display", "role", "timestamp"],
    "键集必须与实时路径一致（多出 JSONL 的 type/id/parentId 就是形状漂移）",
  );
  // 端到端：只读 role/timestamp 的渲染器（实时路径下能画）在读盘路径下也必须能画。
  const fromShape = [];
  buildSessionContext(entries, undefined, {
    messageLines: (message) => (message.role === "custom" && typeof message.timestamp === "number"
      ? (fromShape.push(message.customType), ["✓ shaped"])
      : null),
  });
  assert.deepEqual(fromShape, ["subagent-notify"], "只读 role/timestamp 的渲染器在读盘路径下被跳过 = 形状不一致");
});
