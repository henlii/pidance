...
const {
  ...
  sweepStaleRunningLeases,
} = await jiti.import("./session-running-lease.ts");
const { getRunningStartedAtTable, getRunningRpcSessionIds, getRunningSessionIds, recoverFollowUpQueues } = await jiti.import("./live-session-registry.ts");
...
test("侧栏运行集：仅 host 启动（starting）不点亮徽标，但仍在一般忙碌集里", () => {
  const previousLocks = globalThis.__piStartLocks;
  const previousSessions = globalThis.__piSessions;
  try {
    globalThis.__piStartLocks = new Map([["starting-only", Promise.resolve(null)]]);
    const entry = (sessionId, running) => ({
      sessionId,
      isAlive: () => true,
      isRunning: () => running,
      listPendingExtensionRequests: () => [],
      destroy() {},
    });
    globalThis.__piSessions = new Map([
      ["alive-idle", entry("alive-idle", false)],
      ["really-running", entry("really-running", true)],
    ]);

    const badge = getRunningSessionIds();
    assert.equal(badge.includes("starting-only"), false, "仅 host 启动不该点亮侧栏运行中（实测过一次约 2.5 秒的假运行中）");
    assert.equal(badge.includes("alive-idle"), false, "host 活着但没在跑也不该点亮");
    assert.equal(badge.includes("really-running"), true, "真的在跑的会话要显示运行中");

    // 一般「忙碌」集（归档守卫等在用）仍包含 starting —— 这条是本用例的对照组，
    // 防止有人把两者合成一个而丢掉守卫语义。
    assert.equal(getRunningRpcSessionIds().includes("starting-only"), true, "一般忙碌集仍应包含 starting");
  } finally {
    if (previousLocks === undefined) delete globalThis.__piStartLocks;
    else globalThis.__piStartLocks = previousLocks;
    if (previousSessions === undefined) delete globalThis.__piSessions;
    else globalThis.__piSessions = previousSessions;
  }
});

test("running lease：死持有者的过期文件在列举时被清理", async () => {
...
/** 取得一个确定已死的 pid（容器内 pid 1 存活，不能拿它构造「死持有者」）。 */
...
test("running lease：清理过期文件不误删新 owner，旧 owner 不删新租约", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    writeFileSync(stalePath, JSON.stringify({ pid: await deadPid(), sessionId: "stale", heartbeatAt: 1_000 }));
    writeFileSync(
      freshPath,
...
// ---------------------------------------------------------------------------
// Issue #30 回归：活持有者的写权不得被任何路径夺走
//
// 第一版修复只改了 acquireRunningLease，heartbeat 与清理（list/sweep）仍按
// 「心跳过期」判定，于是活持有者暂停超过 TTL 后，租约仍会被覆盖/删除，
// 随后 acquire 就能接管 —— 实测复现后才补齐这三条路径。
// ---------------------------------------------------------------------------

/** 构造「pid 存活、心跳已过期」的租约；返回 owner pid 与检查/复原函数。 */
async function withStaleLiveLease(sessionId, run) {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-live-"));
  const sleeper = spawnSleeper();
  const dir = join(agentDir, RUNNING_LEASE_DIRNAME);
  const path = join(dir, `${sessionId}.json`);
  const stale = Date.now() - RUNNING_LEASE_TTL_MS - 5_000;
  mkdirSync(dir, { recursive: true });
  const reset = () => writeFileSync(
    path,
    JSON.stringify({ pid: sleeper.pid, sessionId, heartbeatAt: stale, startedAt: stale }),
  );
  const ownerPid = () => {
  ...
  reset();
  try {
    await run({ agentDir, path, reset, ownerPid, stale, ownerPidValue: sleeper.pid });
  } finally {
    sleeper.kill("SIGKILL");
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("running lease：活持有者心跳过期时，list/sweep 不得删除其租约（#30 回归）", async () => {
  const sessionId = "live-stale-list";
  await withStaleLiveLease(sessionId, async ({ agentDir, path, ownerPid, ownerPidValue }) => {
    listFreshRunningLeaseSessionIds(agentDir);
    listFreshRunningLeaseSessions(agentDir);
    assert.equal(existsSync(path), true, "活持有者的租约文件不得被清理删除");
    assert.equal(ownerPid(), ownerPidValue, "owner 不得改变");
    // 关键：文件若被删除，后面的 acquire 就会静默接管（第一版漏洞）
    assert.equal(acquireRunningLease(sessionId, agentDir), false, "清理后仍不得被接管");
  });
});

test("running lease：活持有者心跳过期时，heartbeat 不得覆盖其租约（#30 回归）", async () => {
  const sessionId = "live-stale-heartbeat";
  await withStaleLiveLease(sessionId, async ({ agentDir, ownerPid, ownerPidValue }) => {
    heartbeatRunningLease(sessionId, agentDir);
    assert.equal(ownerPid(), ownerPidValue, "心跳不得把活持有者的租约改成自己");
  });
});

test("running lease：活持有者心跳过期时，isRunningLeaseHeldByOther 仍为真（#30 回归）", async () => {
  const sessionId = "live-stale-query";
  await withStaleLiveLease(sessionId, async ({ agentDir }) => {
    assert.equal(
      isRunningLeaseHeldByOther(sessionId, agentDir),
      true,
      "持有者进程存活即视为被占用，即使心跳过期",
    );
  });
});

test("running lease：活持有者退出后，三条路径都恢复可接管（#30 回归）", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-evict-"));
  ...
  const stale = Date.now() - RUNNING_LEASE_TTL_MS - 5_000;
  mkdirSync(dir, { recursive: true });
  writeFileSync(path, JSON.stringify({ pid: sleeper.pid, sessionId, heartbeatAt: stale, startedAt: stale }));
  const deadPid = sleeper.pid;
  sleeper.kill("SIGKILL");
  await waitForLease(() => {
    try { process.kill(deadPid, 0); return false; } catch { return true; }
  ...
  try {
    assert.equal(isRunningLeaseHeldByOther(sessionId, agentDir), false, "死持有者不再占用");
    assert.deepEqual(listFreshRunningLeaseSessionIds(agentDir), [], "死持有者租约被回收");
    assert.equal(acquireRunningLease(sessionId, agentDir), true, "死持有者退出后可接管");
    releaseRunningLease(sessionId, agentDir);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
...
// ---------------------------------------------------------------------------
// Issue #39：失效租约文件的回收（无界增长）
//
// releaseRunningLease 只在优雅 dispose 时执行；崩溃 / SIGKILL / dispose 未跑到
// 时没有兜底删除。实测本机 2550 个文件、11MB，抽样 300 个全部属于已死进程。
// 回收逻辑一直存在（collectFreshLeases），但两个导出都无生产调用方。
// ---------------------------------------------------------------------------

test("#39 启动回收：删死持有者租约，保留活持有者（含心跳过期）", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-sweep-"));
  const sleeper = spawnSleeper();
  const dir = join(agentDir, RUNNING_LEASE_DIRNAME);
  mkdirSync(dir, { recursive: true });
  ...
  const liveFreshPath = join(dir, "live-fresh.json");
  const liveStalePath = join(dir, "live-stale.json");
  writeFileSync(deadPath, JSON.stringify({ pid: dead, sessionId: "dead", heartbeatAt: stale }));
  writeFileSync(liveFreshPath, JSON.stringify({ pid: sleeper.pid, sessionId: "live-fresh", heartbeatAt: Date.now() }));
  // 活持有者但心跳过期：绝不能删（#30 的规则）
  writeFileSync(liveStalePath, JSON.stringify({ pid: sleeper.pid, sessionId: "live-stale", heartbeatAt: stale }));

  try {
    const result = sweepStaleRunningLeases(agentDir);
    assert.equal(result.skipped, false);
    assert.equal(result.removed, 1, "只应删掉死持有者那一个");
    assert.equal(existsSync(deadPath), false, "死持有者的租约应被回收");
    assert.equal(existsSync(liveFreshPath), true, "活持有者的新鲜租约不得被删");
    assert.equal(existsSync(liveStalePath), true, "活持有者即使心跳过期也不得被删");
    assert.equal(result.active, 1, "active 只计新鲜租约");
    // 回收后仍能正确判定占用
    assert.equal(isRunningLeaseHeldByOther("live-stale", agentDir), true);
  } finally {
    sleeper.kill("SIGKILL");
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("#39 回收不得删掉并发写入的新 owner（同锁内读-改-写）", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-sweep-race-"));
  const sleeper = spawnSleeper();
  const dir = join(agentDir, RUNNING_LEASE_DIRNAME);
  mkdirSync(dir, { recursive: true });
  try {
    // 另一进程（本测试的 sleeper 代表）持有租约后，回收不应删除它
    writeFileSync(
      join(dir, "held.json"),
      JSON.stringify({ pid: sleeper.pid, sessionId: "held", heartbeatAt: Date.now() }),
    );
    sweepStaleRunningLeases(agentDir);
    assert.equal(existsSync(join(dir, "held.json")), true, "活 owner 的租约必须保留");
    assert.equal(isRunningLeaseHeldByOther("held", agentDir), true);
  } finally {
    sleeper.kill("SIGKILL");
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("#39 大量历史文件：一次回收即可清空，且不显著阻塞", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-sweep-bulk-"));
  const dir = join(agentDir, RUNNING_LEASE_DIRNAME);
  mkdirSync(dir, { recursive: true });
  const dead = await deadPid();
  const count = 1_000;
  try {
    for (let i = 0; i < count; i += 1) {
      writeFileSync(
        join(dir, `s-${i}.json`),
        JSON.stringify({ pid: dead, sessionId: `s-${i}`, heartbeatAt: Date.now() }),
      );
    }
    const t0 = Date.now();
    const result = sweepStaleRunningLeases(agentDir);
    const elapsed = Date.now() - t0;
    assert.equal(result.removed, count, `应全部回收，实际 ${result.removed}`);
    // 不设人为上限：用实测固定「一次回收足够快」的结论（本机量级为百毫秒内）
    assert.ok(elapsed < 5_000, `回收 ${count} 个文件耗时 ${elapsed}ms，过慢`);
    assert.deepEqual(listFreshRunningLeaseSessionIds(agentDir), []);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("#39 另一进程持锁时返回 skipped，不抛错", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "lease-sweep-lock-"));
  const dir = join(agentDir, RUNNING_LEASE_DIRNAME);
  mkdirSync(dir, { recursive: true });
  // 用本进程 pid 占住租约锁：活着 → 不会被回收 → withLeaseLock 等待到超时
  writeFileSync(join(dir, ".lease-lock"), JSON.stringify({ pid: process.pid, token: "held" }));
  try {
    const result = sweepStaleRunningLeases(agentDir);
    assert.equal(result.skipped, true, "锁超时不是错误，应返回 skipped");
