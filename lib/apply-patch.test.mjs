...
test("getApplyPatchFailures 逐文件失败：容忍对象与字符串条目，非法输入返回空", async () => {
  const { getApplyPatchFailures } = await loadSubject();
  assert.deepEqual(getApplyPatchFailures(undefined), []);
  assert.deepEqual(getApplyPatchFailures({ result: {} }), []);
  assert.deepEqual(
    getApplyPatchFailures({
      result: {
        failures: [
          { filePath: "src/a.ts", message: "context mismatch" },
          { filePath: "src/b.ts" },
          "src/c.ts",
          "",
          { message: "no path" },
    ...
    [
      ...
      { filePath: "src/b.ts" },
      { filePath: "src/c.ts" },
      { filePath: "", message: "no path" },
    ],
  );
});

test("getApplyPatchAppliedFiles 只取实际写入的路径并去重", async () => {
  const { getApplyPatchAppliedFiles } = await loadSubject();
  assert.deepEqual(getApplyPatchAppliedFiles(undefined), []);
  assert.deepEqual(getApplyPatchAppliedFiles({ result: { appliedFiles: [] } }), []);
  assert.deepEqual(
    getApplyPatchAppliedFiles({ result: { appliedFiles: ["a.ts", "a.ts", "", "  ", "b.ts"] } }),
    ["a.ts", "b.ts"],
  );
});
