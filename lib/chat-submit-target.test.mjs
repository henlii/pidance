...
test("persisted A → new B：旧闭包 current 与 render 构造的 token 不匹配", () => {
  // render 更新后的 currentTargetTokenRef = {new, intent-b}
  const currentToken = captureChatTargetToken({ isNew: true, intentId: "intent-b", persistedSessionId: null });
  const oldSendToken = captureChatTargetToken({ isNew: false, intentId: null, persistedSessionId: "A" });
  assert.equal(sameChatTargetToken(oldSendToken, currentToken), false);
});

test("new A → persisted B：旧 new token 与 render current(persisted B) 不匹配", () => {
  const currentToken = captureChatTargetToken({ isNew: false, intentId: null, persistedSessionId: "B" });
  const oldSendToken = captureChatTargetToken({ isNew: true, intentId: "intent-a", persistedSessionId: null });
  assert.equal(sameChatTargetToken(oldSendToken, currentToken), false);
});

test("persisted A 仍为当前：同 token 匹配", () => {
  const currentToken = captureChatTargetToken({ isNew: false, intentId: null, persistedSessionId: "A" });
  const oldSendToken = captureChatTargetToken({ isNew: false, intentId: null, persistedSessionId: "A" });
  assert.equal(sameChatTargetToken(oldSendToken, currentToken), true);
});
