...
test("SdkSessionHost 队列生命周期：set_follow_up_queue / agent_settled / follow_up_flushed 收口在 Host（源码门禁）", () => {
  const src = readFileSync(new URL("./sdk-session-host.ts", import.meta.url), "utf8");
  assert.match(src, /case "set_follow_up_queue"/);
  assert.match(src, /case "agent_settled"/);
  assert.match(src, /follow_up_flushed/);
  assert.match(src, /readStopReasonFromAgentEnd/);
  assert.match(src, /hydrateFollowUpQueue/);
  assert.match(src, /sessionQueueHold/);
});
