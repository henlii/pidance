...
test("SdkSessionHost：新会话启动、get_state、并发锁与 destroy", async () => {
  ...
  try {
    ...
    // images / imageFallbacks（issue #104 审查 P0-2）：投影必须带上，否则对账/刷新后图消失
    // （文本侧的序列已经被摘走，只剩空行）。没有图时是空数组。
...
// 插件编辑器接管（issue #107）：按键要真的进组件的 handleInput，所以命令必须挂到
// adapter 的 dispatchEditorComponentInput（适配器内部再按 pi-tui 顺序：先全局监听器、
// 后被接管的编辑器）。断言**调用表达式**而不是名字：文件里注释也含这些名字，
// 只匹配名字的话删掉真正的调用测试照样是绿的（反向验证抓到过）。
test("Host 把 editor_component_input 转给 adapter.dispatchEditorComponentInput（issue #107）", () => {
  const src = readFileSync(new URL("./sdk-session-host.ts", import.meta.url), "utf8");
  const at = src.indexOf('case "editor_component_input"');
  assert.notEqual(at, -1, "要有这条命令分支");
  assert.match(
    src,
    /case "editor_component_input"[\s\S]{0,900}dispatchEditorComponentInput\(data, asString\(command\.clientId\) \?\? undefined\)/,
    "命令要落到适配器的输入入口，并把来源标签带下去（否则每个标签都会提交一次）",
  );
  assert.match(src, /case "editor_component_set_text"[\s\S]{0,600}applyEditorTakeoverText\(requestId, text\)/, "回填命令要落到适配器");
  assert.match(src, /case "editor_takeover_dismiss"[\s\S]{0,600}dismissEditorTakeover\(requestId, asString\(command\.clientId\) \?\? undefined\)/, "收起命令要落到适配器（带标签定向）");
});

test("状态投影带 extensionEditorComponent：后加载的页面靠它拿回接管内容（issue #107）", () => {
  const src = readFileSync(new URL("./sdk-session-host.ts", import.meta.url), "utf8");
  assert.match(src, /extensionEditorComponent: this\.extensionUi\?\.editorTakeoverSnapshot \?\? null/, "投影要挂上接管快照");
});
