...
test("A9: 桌面与 390px 视口共用同一 scheduleFrame，无视口专属延迟", async () => {
  function replay(label) {
    ...
    const pending = manager.connect(label, (event) => delivered.push(event.type));
    source.readyState = OPEN;
    source.emit({ type: "connected" });
    source.emit({ type: "message_update", message: { role: "assistant", content: "a" } });
    source.emit({ type: "message_update", message: { role: "assistant", content: "ab" } });
    source.emit({ type: "message_end", message: { role: "assistant", content: "ab" } });
    frames.flush();
    return { pending, delivered, frameCount: 0, frames };
  }
  const desktop = replay("desktop");
  const mobile = replay("mobile-390");
  await desktop.pending;
  await mobile.pending;
  assert.deepEqual(desktop.delivered, mobile.delivered);
  assert.deepEqual(desktop.delivered.filter((type) => type === "message_update"), ["message_update"]);
  assert.ok(desktop.delivered.includes("message_end"));
});
