...
test("A9: 同一录制在桌面(16ms帧)与慢帧(100ms，模拟低端/移动)回放，最终一致且额外延迟 ≤250ms", async () => {
  const recording = [
  ...
  function replay(frameMs) {
    ...
    const firstUpdate = delivered.find((item) => item.type === "message_update");
    return { pending, delivered, endAt: end?.t ?? Infinity, firstUpdateAt: firstUpdate?.t ?? Infinity, lastText: end?.text };
  }

  const desktop = replay(16);
  const mobile390 = replay(100);
  await desktop.pending;
  await mobile390.pending;
  assert.equal(desktop.lastText, "hello");
  assert.equal(mobile390.lastText, "hello");
  // message_update 走 scheduleFrame：慢帧端相对桌面的额外投递延迟受帧间隙约束
  const extraDelay = mobile390.firstUpdateAt - desktop.firstUpdateAt;
  assert.ok(Number.isFinite(extraDelay), "both must deliver message_update");
  assert.ok(extraDelay > 0, "slow frame cadence must show real extra delay, not structural 0");
  assert.ok(extraDelay <= 250, `slow-frame extra delay ${extraDelay}ms exceeds 250ms`);
  // 帧调度是唯一差异：最终消息一致（桌面/移动同路径）
  const finalTexts = (d) => d.delivered.filter((x) => x.type === "message_end").map((x) => x.text);
  assert.deepEqual(finalTexts(desktop), finalTexts(mobile390));
});

// ── #91：长期连接要登记到 live-event-sources（pagehide 集中让出同源连接） ──

test("#91: 会话事件流登记，close() 后注销", async () => {
  const live = await jiti.import("./live-event-sources.ts");
  live.resetLiveEventSourcesForTests();
  const source = createFakeSource();
  const timers = fakeTimers();
  const manager = createEventStreamManager({
    createEventSource: () => source,
  ...
  source.emit({ type: "connected" });
  await promise;
  assert.equal(live.liveEventSourceCount(), 1, "会话事件流必须登记（否则 pagehide 让不出连接）");
  manager.close();
  assert.equal(live.liveEventSourceCount(), 0, "关闭后必须注销，别在登记表里留半状态");
});

test("#91: pagehide 关掉会话事件流；回前台 ensureConnected 重建并重新登记", async () => {
  const live = await jiti.import("./live-event-sources.ts");
  live.resetLiveEventSourcesForTests();
  const sources = [];
  const timers = fakeTimers();
  const manager = createEventStreamManager({
    createEventSource: () => {
      const created = createFakeSource();
  ...
  const first = manager.connect("s1", () => {});
  sources[0].readyState = OPEN;
  sources[0].emit({ type: "connected" });
  await first;
  assert.equal(sources.length, 1);

  live.handlePageHide();
  assert.equal(sources[0].closecalled, true, "pagehide 必须关掉会话事件流（把同源连接让给新文档）");
  assert.equal(live.liveEventSourceCount(), 0);

  const second = manager.ensureConnected("s1", () => {});
  sources[1].readyState = OPEN;
  sources[1].emit({ type: "connected" });
  await second;
  assert.equal(sources.length, 2, "bfcache 恢复后必须重建一条新连接");
  assert.equal(live.liveEventSourceCount(), 1, "重建的连接也要登记");
  manager.close();
});
