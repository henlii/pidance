...
test("标题推导：槽位外壳据此渲染标题行（单 run 用 agent 名、多 run 用通用名）", async () => {
  const snapshot = parseSubagentAsyncSnapshot([line(liveSnapshot())]);
  const single = subagentAsyncHeading(snapshot);
  assert.equal(single.singleLabel, "worker", "单个 run 时应给出它的 label（外壳拼成「异步子代理 worker」）");
  assert.ok(single.running >= 1, "运行中计数应可用（副标题显示「后台」等）");
  assert.equal(typeof single.hidden, "number");
  assert.equal(typeof single.byteLimitExceeded, "boolean");
  const two = subagentAsyncHeading(parseSubagentAsyncSnapshot([line(liveSnapshot({
    runs: [
      { id: "r1", kind: "subagent", label: "worker", state: "running" },
      { id: "r2", kind: "subagent", label: "oracle", state: "queued" },
    ],
  }))]));
  assert.equal(two.singleLabel, null, "多个 run 时不给单名（外壳退回通用标题）");
  assert.equal(two.queued, 1);
});
