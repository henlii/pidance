...
test("宿主把会话草稿读取器接进适配器：少了这行 getEditorText 恒空", () => {
  ...
  // 导入行允许带上同族的备选实现（现在还有 prependComposerDraftText：没人可执行时的兜底）。
  ...
  // 三轮审查 重要 3 的兜底：插件自己调 onSubmit、而当时没有任何标签上报过时，这次提交
  // 只能写进会话草稿（否则静默丢）——这条注入少了就等于又回到「没人执行、字没了」。
  assert.match(
    src,
    /writeComposerDraft: \(text: string\) => \{\s*prependComposerDraftText\(this\.realSessionId, text, this\.agentDir\);/
    ,
    "必须把草稿兜底接进来（读与写都按当前会话 + agentDir）",
  );
});
...
test("编辑器接管：组件调 onSubmit 只发一条提交事件（正文由客户端交给发送管线）", () => {
  ...
  adapter.uiContext.setEditorComponent(() => component);
  // 按键由某个标签发出（真实路径就是这样）：提交只有一个归属者，正好一条。
  ...
  assert.equal(submits[0].clientId, "tab-A", "提交要带上来按键的那个标签");
  assert.equal(
...
test("编辑器接管：onSubmit 与 onChange 由宿主挂到组件实例上（组件自己调 this.onSubmit）", () => {
  ...
  // 插件自己调 onSubmit（不是由某个标签的按键触发）时，宿主按「谁上报过知道自己显示这个
  // 接管」挑归属者（三轮审查 重要 3）：先让本标签上报一次，再验恰好一条、且指向它。
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, true, "tab-A");
...
test("编辑器接管：提交带上「谁敲的字」，落点由请求 id 兜住（issue #107 审查 阻断 2）", () => {
  ...
  // 非按键来源（插件自己调 onSubmit）：宿主挑**一个**归属者，而不是广播（三轮审查 重要 3）。
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, true, "tab-B");
  component.onSubmit("插件自己提交");
  const submits2 = emitted.filter((e) => e.method === "editorComponentSubmit");
  assert.equal(submits2.length, 2);
  assert.equal(submits2.at(-1).clientId, "tab-B", "没有来源时报过视图的标签里挑一个，恰好一条");

  // 两个标签都在显示：仍然只有一条（挑最近上报的那个）。
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, true, "tab-C");
  component.onSubmit("两个标签都在显示");
  const submits3 = emitted.filter((e) => e.method === "editorComponentSubmit");
  assert.equal(submits3.length, 3, "两个标签显示时也只有一条，不是每个标签一条");
  assert.equal(submits3.at(-1).clientId, "tab-C");
});
...
// ---------------------------------------------------------------------------
// 三轮审查新增：卸下时的定向交还、无归属者的提交、空组件与陈旧草稿
// ---------------------------------------------------------------------------

test("编辑器接管：卸下工厂时只把文本交给「刚才在显示接管」的标签（三轮审查 阻断 2）", () => {
  const { adapter, emitted } = makeAdapter();
  const component = Object.assign(new Text("BODY", 0, 0), {
    handleInput() {},
    getText() { return "还没提交的字"; },
  ...
  adapter.uiContext.setEditorComponent(() => component);
  // 两个标签上报：一个在显示（桌面），一个不显示（手机 / 用户点过「返回输入框」）。
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, true, "tab-visible");
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, false, "tab-mobile");
  emitted.length = 0;
  adapter.uiContext.setEditorComponent(undefined);

  const carried = emitted.filter((e) => e.method === "set_editor_text");
  assert.equal(carried.length, 1, "只发给刚才在显示接管的标签，别广播给所有人");
  assert.equal(carried[0].clientId, "tab-visible");
  assert.equal(carried[0].text, "还没提交的字");

  // 对照组：一个标签都没上报过（老客户端 / 刚连上还没渲染）→ 退回广播，别让文本消失。
  const second = makeAdapter();
  const comp2 = Object.assign(new Text("BODY", 0, 0), { handleInput() {}, getText() { return "兜底的字"; } });
  second.adapter.uiContext.setEditorComponent(() => comp2);
  second.emitted.length = 0;
  second.adapter.uiContext.setEditorComponent(undefined);
  const fallback = second.emitted.filter((e) => e.method === "set_editor_text");
  assert.equal(fallback.length, 1, "没人上报时仍要交还（否则文本随组件一起消失）");
  assert.equal(fallback[0].clientId, undefined, "没有定向对象，只能广播");
});

test("编辑器接管：只显示给「上报过在显示」的标签的那条文本，不会漏给多个标签（对照组）", () => {
  // 两个标签**都**在显示：各发一条定向文本（客户端按 clientId 各取各的）。
  const { adapter, emitted } = makeAdapter();
  const component = Object.assign(new Text("BODY", 0, 0), { handleInput() {}, getText() { return "给两个标签"; } });
  adapter.uiContext.setEditorComponent(() => component);
  const id = adapter.editorTakeoverSnapshot.id;
  adapter.recordEditorTakeoverView(id, true, "tab-1");
  adapter.recordEditorTakeoverView(id, true, "tab-2");
  emitted.length = 0;
  adapter.uiContext.setEditorComponent(undefined);
  const carried = emitted.filter((e) => e.method === "set_editor_text");
  assert.deepEqual(carried.map((e) => e.clientId).sort(), ["tab-1", "tab-2"]);
});

test("编辑器接管：插件自己提交、而没有任何标签上报过 → 落进会话草稿（三轮审查 重要 3）", () => {
  const drafts = [];
  const { adapter, emitted } = makeAdapter({ writeComposerDraft: (text) => drafts.push(text) });
  const component = Object.assign(new Text("BODY", 0, 0), {
    handleInput() {},
    onSubmit: undefined,
    getText() { return "x"; },
  ...
  component.onSubmit("没人可以执行的提交");
  assert.equal(
    emitted.filter((e) => e.method === "editorComponentSubmit").length,
    0,
    "没有归属者时不该广播（广播会在每个标签各执行一次）",
  );
  assert.deepEqual(drafts, ["没人可以执行的提交"], "也不能静默丢：写进草稿，用户下次打开会话能看到");

  // 对照组：有标签上报过 → 定向执行，且**不**写草稿。
  adapter.recordEditorTakeoverView(adapter.editorTakeoverSnapshot.id, true, "tab-A");
  component.onSubmit("有归属者的提交");
  const submits = emitted.filter((e) => e.method === "editorComponentSubmit");
  assert.equal(submits.length, 1);
  assert.equal(submits[0].clientId, "tab-A");
  assert.deepEqual(drafts, ["没人可以执行的提交"], "定向执行了就不该再写一份草稿（两处都有会分叉）");
});

test("编辑器接管：已收起的标签也参与兜底（它有输入框，能执行），但排在显示中的之后", () => {
  const { adapter, emitted } = makeAdapter();
  const component = Object.assign(new Text("BODY", 0, 0), { handleInput() {}, getText() { return "x"; } });
  adapter.uiContext.setEditorComponent(() => component);
  const id = adapter.editorTakeoverSnapshot.id;
  // 先上报「收起的」（shown=false），再上报「显示中的」：兜底应当挑显示中的那个。
  adapter.recordEditorTakeoverView(id, false, "tab-dismissed");
  adapter.recordEditorTakeoverView(id, true, "tab-shown");
  component.onSubmit("兜底");
  assert.equal(emitted.filter((e) => e.method === "editorComponentSubmit").at(-1).clientId, "tab-shown");

  // 「全部收起」：没有任何标签在显示接管，但收起的标签也有输入框 —— 提交仍要执行（三轮审查的验收项）。
  const allDismissed = makeAdapter();
  const comp2 = Object.assign(new Text("BODY", 0, 0), { handleInput() {}, getText() { return "x"; } });
  allDismissed.adapter.uiContext.setEditorComponent(() => comp2);
  allDismissed.adapter.recordEditorTakeoverView(allDismissed.adapter.editorTakeoverSnapshot.id, false, "tab-mobile");
  comp2.onSubmit("全部收起时也要执行");
  const submits = allDismissed.emitted.filter((e) => e.method === "editorComponentSubmit");
  assert.equal(submits.length, 1, "全都收起时提交不能静默丢");
  assert.equal(submits[0].clientId, "tab-mobile", "交给收起的那个标签（它看得见输入框）");
});

test("编辑器接管：只有最近还在心跳的标签当归属者，过期登记不再接收提交（四轮审查 阻断 1）", () => {
  let t = 1_000_000;
  const drafts = [];
  const { adapter, emitted } = makeAdapter({ now: () => t, writeComposerDraft: (text) => drafts.push(text) });
  const component = Object.assign(new Text("BODY", 0, 0), { handleInput() {} });
  adapter.uiContext.setEditorComponent(() => component);
  const id = adapter.editorTakeoverSnapshot.id;
  adapter.recordEditorTakeoverView(id, true, "tab-A");

  // 心跳还在：正常挑到 A（恰好一条，定向）。
  component.onSubmit("心跳还在时的提交");
  const first = emitted.filter((e) => e.method === "editorComponentSubmit");
  assert.equal(first.length, 1);
  assert.equal(first[0].clientId, "tab-A");

  // 心跳停了（切走 / 关标签 / 崩溃）：过了新鲜度窗口就不该再定向给它 ——
  // 那个标签已经不订阅这条流，定向过去只会静默丢字。
  t += 60_000;
  component.onSubmit("心跳停了之后的提交");
  assert.equal(
    emitted.filter((e) => e.method === "editorComponentSubmit").length,
    1,
    "过期登记不能再被选中（否则提交被定向到一个已离开的标签）",
  );
  assert.deepEqual(drafts, ["心跳停了之后的提交"], "挑不到活着的标签时落到**持久**草稿，不静默丢");
});

test("编辑器接管：还在心跳的标签胜过停了的登记（心跳把活标签顶成最近上报者）", () => {
  let t = 1_000_000;
  const { adapter, emitted } = makeAdapter({ now: () => t });
  const component = Object.assign(new Text("BODY", 0, 0), { handleInput() {} });
  adapter.uiContext.setEditorComponent(() => component);
  const id = adapter.editorTakeoverSnapshot.id;

  adapter.recordEditorTakeoverView(id, true, "tab-old");
  t += 30_000; // tab-old 还没过期，但此后不再心跳
  adapter.recordEditorTakeoverView(id, true, "tab-live"); // tab-live 每 15s 心跳一次
  component.onSubmit("两个都新鲜时挑最近上报的");
  assert.equal(
    emitted.filter((e) => e.method === "editorComponentSubmit").at(-1)?.clientId,
    "tab-live",
    "活标签的心跳让它成为最近上报者（过期登记不会因为序号更大而被选中）",
  );
});

test("编辑器接管：卸下时只交给还在心跳的「正在显示」标签，否则广播 + 写一份持久草稿", () => {
  let t = 1_000_000;
  const drafts = [];
  const { adapter, emitted } = makeAdapter({ now: () => t, writeComposerDraft: (text) => drafts.push(text) });
  const component = Object.assign(new Text("BODY", 0, 0), { getText: () => "留在组件里的字" });
  adapter.uiContext.setEditorComponent(() => component);
  const id = adapter.editorTakeoverSnapshot.id;
  adapter.recordEditorTakeoverView(id, true, "tab-gone");
  t += 60_000; // 心跳停了

  adapter.uiContext.setEditorComponent(undefined);
  const sets = emitted.filter((e) => e.method === "set_editor_text");
  assert.equal(sets.length, 1, "退回一条广播（不是定向给已离开的标签）");
  assert.equal(sets[0].clientId, undefined, "已不新鲜的登记不能再作为交还目标");
  assert.deepEqual(drafts, ["留在组件里的字"], "同一帧卸载/挂载时广播未必落得下 → 同时写持久草稿");

  // 对照组：心跳还在时仍然只定向给那个标签，且不写草稿（确认上一条不是恒真）。
  const fresh = makeAdapter({ now: () => t, writeComposerDraft: (text) => drafts.push(text) });
  const live = Object.assign(new Text("BODY", 0, 0), { getText: () => "仍在显示" });
  fresh.adapter.uiContext.setEditorComponent(() => live);
  const liveId = fresh.adapter.editorTakeoverSnapshot.id;
  fresh.adapter.recordEditorTakeoverView(liveId, true, "tab-here");
  fresh.adapter.uiContext.setEditorComponent(undefined);
  const freshSets = fresh.emitted.filter((e) => e.method === "set_editor_text");
  assert.equal(freshSets.length, 1);
  assert.equal(freshSets[0].clientId, "tab-here", "新鲜的登记照旧定向");
  assert.deepEqual(drafts, ["留在组件里的字"], "定向成功那条不写草稿（否则用户会看到两份）");
});

test("编辑器接管：getEditorText 认「组件在且为空」，不回退到陈旧的草稿镜像（三轮审查 重要 5）", () => {
  const { adapter } = makeAdapter({ readComposerText: () => "输入框里的旧草稿" });
  const empty = Object.assign(new Text("BODY", 0, 0), { handleInput() {}, getText() { return ""; } });
  adapter.uiContext.setEditorComponent(() => empty);
  assert.equal(
    adapter.uiContext.getEditorText(),
    "",
    "接管期间「编辑器」就是这个空组件：回退到 400ms 防抖的草稿镜像会让插件把空编辑器判成有字",
  );

  // 对照组：没有接管时照旧读草稿镜像。
  const bare = makeAdapter({ readComposerText: () => "输入框里的旧草稿" });
  assert.equal(bare.adapter.uiContext.getEditorText(), "输入框里的旧草稿");
});
