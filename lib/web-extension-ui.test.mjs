...
test("没有等价语义的 UI 能力：不静默 no-op，但同一能力只提示一次", () => {
  const { adapter, emitted } = makeAdapter();
  // setFooter / setHeader（issue #98）与 addAutocompleteProvider（issue #101）都已实现，
  // 用仍未实现的 setEditorComponent 守「同一能力只提示一次」这条。
  adapter.uiContext.setEditorComponent(() => ({ render: () => ["z"] }));
  adapter.uiContext.setEditorComponent(() => ({ render: () => ["w"] }));

  const notices = emitted.filter((e) => e.method === "notify" && e.notifyType === "warning");
  assert.equal(notices.length, 1, "同一能力多次调用只提示一次");
  assert.match(notices[0].message, /setEditorComponent/);
});
...
test("补全：坏工厂被跳过，不影响整链（一个坏插件不该让补全整体失效）", async () => {
  ...
  assert.equal(emitted.filter((e) => e.method === "autocompleteProviders").at(-1).count, 2);
});
