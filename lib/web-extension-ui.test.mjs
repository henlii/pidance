...
test("编辑器接管：接管期间 setEditorText 写进组件（不再发给已让位的输入框）", () => {
  const written = [];
  const { adapter, emitted } = makeAdapter();
  const component = Object.assign(new Text("BODY", 0, 0), {
    handleInput() {},
  ...
  assert.equal(
    emitted.filter((e) => e.method === "set_editor_text").length,
    before,
    "接管时不该再往客户端发 set_editor_text（输入框不存在，会被静默丢掉）",
  );
  assert.ok(
  ...
  adapter2.uiContext.setEditorText("普通路径");
  const sent = emitted2.filter((e) => e.method === "set_editor_text");
  assert.equal(sent.length, 1);
  assert.equal(sent[0].text, "普通路径");
});
