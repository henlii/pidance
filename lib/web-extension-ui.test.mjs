...
test("theme 接口形状与真 Theme 一致，未知成员仍可调用不抛错", () => {
  const { adapter } = makeAdapter();
  const theme = adapter.uiContext.theme;
  // 真成员必须是正确类型：以前一律返回透传函数时，theme.name 是函数、
  // getThinkingBorderColor("high") 返回字符串，插件一调用就 TypeError。
  ...
  assert.equal(theme.getThinkingBorderColor("high")("z"), "z");
  assert.equal(typeof theme.getBashModeBorderColor(), "function");
  for (const style of ["bold", "dim", "italic", "underline", "inverse", "strikethrough"]) {
    assert.equal(theme[style]("T"), "T", style);
  }
  // 未知成员不能是 undefined：插件会自造 theme.border/hint 这类包装去调它，
  // 返回 undefined 会从「没颜色但文本还在」退化成直接抛错。
  assert.equal(theme.notARealMember("still here"), "still here");
...
// ---------------------------------------------------------------------------
// 渲染宽度：前端上报可用列数，已挂载的插件界面按新宽度重排
// ---------------------------------------------------------------------------

test("setRenderWidth：同宽度不重排，非法宽度忽略", () => {
  const { adapter } = makeAdapter();
  assert.equal(adapter.setRenderWidth(80), true, "第一次设置算变化");
  assert.equal(adapter.setRenderWidth(80), false);
  assert.equal(adapter.setRenderWidth(Number.NaN), false);
  assert.equal(adapter.setRenderWidth(0), false);
  assert.equal(adapter.setRenderWidth(-3), false);
});

test("setRenderWidth：宽度变化让 custom 面板与 widget 用新宽度重渲", async () => {
  const { adapter, emitted } = makeAdapter();
  adapter.setRenderWidth(80);
