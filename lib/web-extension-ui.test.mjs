...
const { setPiThemeConstructor } = await jiti.import("./tui-render-bridge.ts");
const { Theme: SdkTheme } = await import("@earendil-works/pi-coding-agent");
setPiThemeConstructor(SdkTheme);
const { createWebExtensionUIAdapter, createFallbackThemeStub, appendCapabilityNotice, MAX_CAPABILITY_NOTICES, normalizeDialogTimeout, MAX_DIALOG_TIMEOUT_MS } = await jiti.import("./web-extension-ui.ts");
...
test("能力提示留成只读快照：只记宿主自己发的，不记插件的 notify", () => {
  ...
  // setFooter / setHeader 在 issue #98 已实现，这里改用仍然没有等价语义的能力来验证重放集合。
  adapter.uiContext.setEditorComponent(() => ({ render: () => ["x"] }));
  const notices = adapter.capabilityNoticeSnapshot;
  assert.equal(notices.length, 1, "不支持的 setEditorComponent 应留下一条能力提示");
  assert.equal(notices[0].notifyType, "warning");
  assert.match(notices[0].message, /"setEditorComponent" is not supported/);
...
test("能力提示快照有上限：超出保留最新，状态不会被撑到无界（issue #93 P2）", () => {
  // 取舍写在 MAX_CAPABILITY_NOTICES 的注释里：宿主能力提示的种类是枚举（当前 7 种），
...
test("没有等价语义的 UI 能力：不静默 no-op，但同一能力只提示一次", () => {
  const { adapter, emitted } = makeAdapter();
  // setFooter / setHeader 已实现（issue #98）；用仍未实现的能力守「只提示一次」这条。
  adapter.uiContext.addAutocompleteProvider(() => ({ getSuggestions: async () => [] }));
  adapter.uiContext.addAutocompleteProvider(() => ({ getSuggestions: async () => [] }));
  adapter.uiContext.setEditorComponent(() => ({ render: () => ["z"] }));

  const notices = emitted.filter((e) => e.method === "notify" && e.notifyType === "warning");
  assert.equal(notices.length, 2, "addAutocompleteProvider 两次只提示一次，setEditorComponent 另计");
  assert.match(notices[0].message, /addAutocompleteProvider/);
...
test("setFooter 的第三个参数（footer 数据）不传：SDK 的 ReadonlyFooterDataProvider 我们没有等价物", () => {
  const { adapter } = makeAdapter();
  const footer = mountSlotOn(adapter, "footer", () => ["f"]);
  assert.equal(footer.footerData, undefined, "宁可不传，也不塞一个缺成员的假对象");
});
