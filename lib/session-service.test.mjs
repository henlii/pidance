...
test("leaf sidecar 防过期：agent_end 清 sidecar，末尾导航清过期 sidecar（静态门禁）", async () => {
  ...
  // 目标是文件末尾 = 外部 pi 默认 leaf：不得只跳过写入（会残留旧分支指针，
  // 下次磁盘 open 恢复旧 leaf，导航到最新分支的意图丢失），必须清 sidecar。
  assert.match(
    svc,
  ...
  assert.match(
    external,
    /if \(sessionManager\.getLastEntryId\(\) !== targetId\) \{[\s\S]*?clearLeafSidecar\(this\.realSessionFile\)/,
  );
});
