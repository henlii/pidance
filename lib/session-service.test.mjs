...
test("appendActivity：外部 RPC live 会话先 destroy 再写盘（单写者静态门禁）", async () => {
  ...
  const openIdx = svc.indexOf("const manager = openSessionView(filePath)");
...
test("httpStatusForSessionError：只读 403、缺失 404、写锁 409", () => {
  ...
  assert.equal(
    httpStatusForSessionError(
      new Error("Session is locked by another Pidance process (writable host ownership)"),
    ),
    409,
  );
