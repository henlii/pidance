...
test("listSessions 聚合会话列表与运行中 id（只算「真的在跑」，不含仅 host 启动）", async () => {
  const service = createSessionService({
    listAllSessions: async () => [{ id: "s1", cwd: "/tmp", path: "/tmp/s1.jsonl", created: "", modified: "", messageCount: 0, firstMessage: "" }],
    // 一般忙碌集包含「仅 host 启动」的 s2，徽标集不含 —— 聚合必须用后者，
    // 否则打开一个空闲会话会让它在侧栏闪出「运行中」约 2.5 秒（用户实测）。
    getRunningRpcSessionIds: () => ["s1", "s2-starting"],
    getRunningSessionIds: () => ["s1"],
  });

  const result = await service.listSessions();
  assert.equal(result.sessions.length, 1);
  assert.deepEqual(result.runningSessionIds, ["s1"], "聚合里的运行中 id 只算真的在跑");
});
...
test("getContextPage / getNavigationSnapshot：自定义消息经注入渲染器带上渲染行（issue #76）", async () => {
  const dir = mkdtempSync(join(tmpdir(), "pidance-plugin-message-"));
  try {
    const filePath = join(dir, "session.jsonl");
    const entries = [
      { type: "session", version: 3, id: "qa-plugin-message-session", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir },
      {
        type: "message",
        id: "u1",
        parentId: null,
      ...
      },
      {
        ...
        content: "raw notify text",
        display: true,
        details: { status: "completed", agent: "worker" },
      },
      // 内置的 custom_message（compaction）由客户端专门视图渲染，不该去解析插件渲染器
      {
        type: "compaction",
        id: "cmp1",
        parentId: "m1",
    ...
    const withRenderer = createSessionService({
      resolveSessionPath: async () => filePath,
      getRpcSession: () => undefined,
      resolveMessageLinesProvider: async () => (message) => {
        calls.push(message.customType);
        return [`rendered ${message.customType}`];
      },
    });
    const page = await withRenderer.getContextPage("qa-plugin-message-session", { limit: 50 });
    const projected = page.context.messages.find((m) => m.customType === "subagent-notify");
    assert.ok(projected, "自定义消息必须在时间线里");
    assert.deepEqual(projected.renderedLines, ["rendered subagent-notify"]);
    assert.equal(projected.content, "raw notify text", "原文仍保留（客户端回退用）");
    assert.deepEqual(calls, ["subagent-notify"], "内置 custom_message 不交给插件渲染器");

    // 导航快照（首屏路径）同一口径
    const snapshot = await withRenderer.getNavigationSnapshot("qa-plugin-message-session", {});
    const snapshotMessage = snapshot.context.messages.find((m) => m.customType === "subagent-notify");
    assert.deepEqual(snapshotMessage.renderedLines, ["rendered subagent-notify"]);

    // 解析不到渲染器 → 退回原文（信息不丢，与 entry 侧「不投影」不同）
    const withoutRenderer = createSessionService({
      resolveSessionPath: async () => filePath,
      getRpcSession: () => undefined,
      resolveMessageLinesProvider: async () => null,
    });
    const plainPage = await withoutRenderer.getContextPage("qa-plugin-message-session", { limit: 50 });
    const plain = plainPage.context.messages.find((m) => m.customType === "subagent-notify");
    assert.equal(plain.renderedLines, undefined);
    assert.equal(plain.content, "raw notify text");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
/**
 * 投影行随「当前主题」重算，而不是被缓存冻住（issue #109 的前提）。
 *
 * 客户端切主题后靠**重拉一页**换色，这条前提必须成立：真实路径里 `resolveMessageLinesProvider`
 * 每次请求都重新解析，而它是在**解析时**就地取当前主题（`loadPiTheme()`）——所以第二次读
 * 会带着新主题渲染。要是有人把 provider 或整页投影缓存起来，#109 的修复会静默失效，
 * 这条守的就是它（这里也顺带走了一次**只读缓存**路径：文件已被缓存，颜色仍要跟着主题走）。
 */
test("#109 重拉一页必须换到新主题色：投影按请求重新解析 provider", async () => {
  const { loadPiTheme, setCurrentPiTheme, resetPiThemeForTests } = await jiti.import("./tui-render-bridge.ts");
  const dir = mkdtempSync(join(tmpdir(), "pidance-theme-projection-"));
  const filePath = join(dir, "session.jsonl");
  const entries = [
    { type: "session", version: 3, id: "qa-theme-projection", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir },
    {
      type: "message",
      id: "u1",
      parentId: null,
  ...
  const service = createSessionService({
    resolveSessionPath: async () => filePath,
    getRpcSession: () => undefined,
    // 与真实实现同口径：**在解析时**取当前主题，渲染时用它
    resolveMessageLinesProvider: async () => {
      const theme = loadPiTheme();
      resolutions.push(theme?.name ?? "none");
      const name = theme?.name ?? "none";
      return () => [`[${name}] rendered`];
    },
  });
  try {
    setCurrentPiTheme({ name: "dark" });
    const first = (await service.getContextPage("qa-theme-projection", { limit: 50 }))
      .context.messages.find((m) => m.customType === "theme-probe");
    assert.deepEqual(first.renderedLines, ["[dark] rendered"]);

    // 切主题后重拉一页：必须换成新色，而不是命中旧投影
    setCurrentPiTheme({ name: "light" });
    const second = (await service.getContextPage("qa-theme-projection", { limit: 50 }))
      .context.messages.find((m) => m.customType === "theme-probe");
    assert.deepEqual(second.renderedLines, ["[light] rendered"], "重拉一页必须带上新主题");
    assert.deepEqual(resolutions, ["dark", "light"], "每次请求都重新解析 provider，缓存不得冻住颜色");

    // 首屏路径同一口径
    const snapshot = await service.getNavigationSnapshot("qa-theme-projection", {});
    const navMessage = snapshot.context.messages.find((m) => m.customType === "theme-probe");
    assert.deepEqual(navMessage.renderedLines, ["[light] rendered"]);
  } finally {
    resetPiThemeForTests();
    rmSync(dir, { recursive: true, force: true });
  }
});
