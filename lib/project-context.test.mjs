...
test('shouldApplyWorktreeIdentityPatch：只回写仍停留在该项目的快照', () => {
  assert.equal(project.shouldApplyWorktreeIdentityPatch({
    snapshotCwd: '/repo-b', snapshotProjectRoot: '/repo-b', requestedRoot: '/repo-b', canonicalRoot: '/repo-b',
  }), true);
  assert.equal(project.shouldApplyWorktreeIdentityPatch({
    snapshotCwd: '/repo-a/wt', snapshotProjectRoot: '/repo-a', requestedRoot: '/repo-a', canonicalRoot: '/repo-a',
  }), true);
  // 已切到另一项目：迟到的旧预加载不得回写
  assert.equal(project.shouldApplyWorktreeIdentityPatch({
    snapshotCwd: '/repo-a', snapshotProjectRoot: '/repo-a', requestedRoot: '/repo-b', canonicalRoot: '/repo-b',
  }), false);
  assert.equal(project.shouldApplyWorktreeIdentityPatch({
    snapshotCwd: null, snapshotProjectRoot: null, requestedRoot: '/repo-b', canonicalRoot: '/repo-b',
  }), false);
});
