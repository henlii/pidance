...
test('项目排序、去重与路径显示', () => {
  assert.deepEqual(project.getRecentProjects([session('a', '2024-01-01'), session('b', '2024-02-01'), session('a2', '2024-03-01', { cwd: '/repo/a' })]), ['/repo/a', '/repo/b']);
...
test('store 保持身份切换、归一化、重置与引用稳定性', () => {
  const store = project.createProjectStore({ identity: { cwd: '/x', status: 'ready' } });
  const initial = store.getIdentitySnapshot();
  assert.deepEqual(initial, { status: 'ready', error: null, cwd: '/x', projectRoot: '/x' });
  let notifications = 0;
  store.subscribeIdentity(() => notifications++);
  store.setIdentity({ status: 'validating' });
  assert.equal(notifications, 1);
  assert.equal(store.getIdentitySnapshot().cwd, '/x');
  assert.equal(store.getIdentitySnapshot().projectRoot, '/x');
  // projectRoot 恒等于 cwd：显式传入旧的项目根也会被归一化
  store.setIdentity({ cwd: '/y', projectRoot: '/root', status: 'ready' });
  assert.equal(notifications, 2);
  assert.equal(store.getIdentitySnapshot().projectRoot, '/y');
  store.setIdentity({ cwd: null });
  assert.deepEqual(store.getIdentitySnapshot(), { status: 'ready', error: null, cwd: null, projectRoot: null });
  assert.equal(notifications, 3);
  store.reset();
  assert.deepEqual(store.getIdentitySnapshot(), { status: 'idle', error: null, cwd: null, projectRoot: null });
