...
test("无门禁资源且 projectTrusted=false 时不扫项目 .agents/skills", () => {
  const dir = tmpdir();
  const agentDir = path.join(dir, "agent");
  const cwd = path.join(dir, "proj");
  fs.mkdirSync(cwd, { recursive: true });
  writeSkill(path.join(cwd, ".agents", "skills", "proj-skill", "SKILL.md"), "proj-skill");
...
test("projectTrusted=true 时向上收集祖先 .agents/skills（排除用户全局）", () => {
  ...
  const { skills } = discoverSkills({ cwd, agentDir, projectTrusted: true, homeDir });
