...
test("上下文：只有 entryId 命中的那条带 isStreaming（其余是历史）", () => {
  const { transformer, calls } = recordingTransformer();
  invalidateMarkdownTransformCache();
  const input = {
    messages: [
  ...
  transformContextMarkdown(input, {
    chain: chainOf([transformer]),
    availableWidth: 100,
    streamingEntryId: "e2",
  });
  assert.deepEqual(
    calls.map((call) => ({ text: call.markdown, isStreaming: call.isStreaming })),
    [{ text: "old", isStreaming: false }, { text: "live", isStreaming: true }],
  );
});
