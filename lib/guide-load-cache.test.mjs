...
// ── 引导页目标 → 项目下拉选择（同一实例内的外部切目标）──

const PROJECTS = [{ cwd: "/repo" }, { cwd: "/other" }];

test("resolveGuideTargetSync 外部切到另一项目：空态与已选态都切过去", () => {
  // 首次（无选择）：从 intent cwd 恢复
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/repo", null), { kind: "load", root: "/repo" });
  // issue 复现路径：引导页已选 A，会话列表点 B 的新建会话 → 必须切到 B
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/other", "/repo"), { kind: "load", root: "/other" });
});

test("resolveGuideTargetSync 同项目换工作树/子目录不重载分支列表", () => {
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/repo-worktrees/feat", "/repo"), { kind: "keep" });
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/repo/sub", "/repo"), { kind: "keep" });
});

test("resolveGuideTargetSync 目标为空：已选则回到未选择态", () => {
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, null, "/repo"), { kind: "clear" });
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, null, null), { kind: "keep" });
});

test("resolveGuideTargetSync 项目列表后到：先保持、列表到达后再切", () => {
  // fetch /api/sessions 未返回（projects 为空）：无匹配不当作已选，等列表到达
  assert.deepEqual(resolveGuideTargetSync([], "/repo", null), { kind: "keep" });
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/repo", null), { kind: "load", root: "/repo" });
});

test("resolveGuideTargetSync 目标不在列表内：清掉旧选择，不伪造项目条目", () => {
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/unlisted", "/repo"), { kind: "clear" });
  assert.deepEqual(resolveGuideTargetSync(PROJECTS, "/unlisted", null), { kind: "keep" });
});
