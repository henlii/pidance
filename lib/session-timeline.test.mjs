...
test("引导的确认不被别的提交 key 领走：不覆盖他人气泡，且引导自身被确认", () => {
  // 生产 SSE 不带 submissionId，提交匹配是 FIFO 的：引导（无 submission）的
  // message_end 会先命中前一个还挂着 key 的提交。
  const timeline = [
    ...timelineFromDisk([user("旧的提问"), assistant("旧回答")], ["e1", "e2"]),
    optimisticRecord(submissionKey("s1"), user("本轮提问")),
    { ...optimisticRecord("local:steer", user("引导：换个方向")), duringStreamingStep: true },
  ];
  const result = confirmUserMessage(timeline, {
    key: submissionKey("s1"),
    message: user("引导：换个方向"),
    entryId: "",
    fallbackKey: "local:x",
  });
  assert.equal(result.outcome, "text");
  assert.equal(result.timeline.length, timeline.length, "不追加");
  assert.equal(
    result.timeline.find((record) => record.key === submissionKey("s1"))?.message.content,
    "本轮提问",
    "别人的气泡不得被改写",
  );
  assert.equal(
    result.timeline.find((record) => record.key === "local:steer")?.pending,
    false,
    "引导必须被确认，否则会一直以乐观气泡身份排到 live 之后",
  );
});

test("磁盘已有同文历史且 key 未命中时，仍要确认同文引导（不得只报 reconciled）", () => {
  const timeline = [
    ...timelineFromDisk([user("引导：换个方向"), assistant("收到")], ["e1", "e2"]),
    { ...optimisticRecord("local:steer", user("引导：换个方向")), duringStreamingStep: true },
  ];
  const result = confirmUserMessage(timeline, {
    key: submissionKey("stale"),
    message: user("引导：换个方向"),
    entryId: "",
    fallbackKey: "local:y",
  });
  assert.equal(result.outcome, "text");
  assert.equal(result.timeline.length, timeline.length, "不得追加重复");
  assert.deepEqual(timelineEntryIds(result.timeline), ["e1", "e2", ""]);
  assert.equal(result.timeline.at(-1).pending, false);
});

test("key 指向的记录已确认过时，不覆盖它，改为确认同文引导", () => {
  const timeline = [
    { key: submissionKey("old"), message: user("上一轮提问"), entryId: "", pending: false },
    { ...optimisticRecord("local:steer", user("引导：换个方向")), duringStreamingStep: true },
  ];
  const result = confirmUserMessage(timeline, {
    key: submissionKey("old"),
    message: user("引导：换个方向"),
    entryId: "",
    fallbackKey: "local:z",
  });
  assert.equal(result.outcome, "text");
  assert.equal(result.timeline[0].message.content, "上一轮提问");
  assert.equal(result.timeline.at(-1).pending, false);
});

// ── issue #109：主题刷新的「按 entryId 只换 renderedLines」 ────────────────────

const customMessage = (lines, customType = "qa") => ({
  role: "custom",
  customType,
  content: "probe",
  display: true,
...
const customRecord = (entryId, lines) => ({
  key: entryId,
  message: customMessage(lines),
  entryId,
  pending: false,
});

test("主题刷新：只换匹配 entryId 的 renderedLines，顺序/长度/身份一概不动", () => {
  const timeline = [
    customRecord("e1", ["old-1"]),
    diskRecordUser("e2", "hi"),
    customRecord("e3", ["old-3"]),
  ];
  const next = replaceRenderedLinesByEntryId(
    timeline,
    [customMessage(["new-1"]), customMessage(["new-3"])],
    ["e1", "e3"],
  );
  assert.notEqual(next, timeline, "有替换时必须换新数组（否则 publish 看不到变化）");
  assert.deepEqual(next.map((record) => record.entryId), ["e1", "e2", "e3"]);
  assert.deepEqual(next.map((record) => record.key), ["e1", "e2", "e3"]);
  assert.equal(next.length, timeline.length);
  assert.deepEqual(next[0].message.renderedLines, ["new-1"]);
  assert.deepEqual(next[2].message.renderedLines, ["new-3"]);
  assert.equal(next[1], timeline[1], "未匹配的记录保持原对象引用");
  assert.equal(timeline[0].message.renderedLines[0], "old-1", "不得就地改写原时间线");
});

test("主题刷新：响应里多出来的尾页记录不会被塞进时间线（窗口不被换成尾页）", () => {
  // 这是本 issue 的核心回归点：走 mergeTailRecords 时新尾页会把窗口整段替换掉。
  const timeline = [customRecord("e1", ["old-1"]), customRecord("e5", ["old-5"])];
  const next = replaceRenderedLinesByEntryId(
    timeline,
    [customMessage(["new-1"]), customMessage(["new-5"]), customMessage(["brand-new"])],
    ["e1", "e5", "e9"],
  );
  assert.deepEqual(next.map((record) => record.entryId), ["e1", "e5"], "不得追加响应里的新记录");
  assert.equal(next.length, 2);
  assert.deepEqual(next[1].message.renderedLines, ["new-5"]);
});

test("主题刷新：没有匹配项 / 行没变 / 服务端没给合法行时返回原引用（不做无谓发布）", () => {
  const timeline = [customRecord("e1", ["same"]), customRecord("e2", ["keep-me"])];
  assert.equal(
    replaceRenderedLinesByEntryId(timeline, [customMessage(["x"])], ["zzz"]),
    timeline,
    "没有匹配 entryId",
  );
  assert.equal(
    replaceRenderedLinesByEntryId(timeline, [customMessage(["same"])], ["e1"]),
    timeline,
    "行完全一致",
  );
  assert.equal(
    replaceRenderedLinesByEntryId(timeline, [customMessage([])], ["e1"]),
    timeline,
    "空数组不是合法行（validRenderedLines 判空）",
  );
  assert.equal(
    replaceRenderedLinesByEntryId(timeline, [user("plain")], ["e1"]),
    timeline,
    "非 custom 消息没有 renderedLines",
  );
  const next = replaceRenderedLinesByEntryId(timeline, [customMessage(["x"])], ["e1"]);
  assert.deepEqual(next[1].message.renderedLines, ["keep-me"], "没给出的那条必须保持原样（不清空覆盖层）");
});

test("主题刷新：pending / duringStreamingStep 等记录标记原样保留", () => {
  const timeline = [{
    key: "local-1",
    message: { ...customMessage(["old"]), renderedLines: undefined },
    entryId: "e7",
    pending: true,
    duringStreamingStep: true,
  }];
  const next = replaceRenderedLinesByEntryId(timeline, [customMessage(["new"])], ["e7"]);
  assert.equal(next[0].pending, true);
  assert.equal(next[0].duringStreamingStep, true);
  assert.equal(next[0].key, "local-1");
  assert.deepEqual(next[0].message.renderedLines, ["new"]);
});

test("主题刷新：collectTimelineRenderedLines 收「当前行」——同 id 取第一条、没有合法行的跳过", () => {
  const timeline = [
    customRecord("e1", ["old-1"]),
    customRecord("e1", ["不该赢-同 id 第二条"]),
    diskRecordUser("e2", "hi"),
    customRecord("e3", ["old-3"]),
    customRecord("e4", []),
  ];
  const lines = collectTimelineRenderedLines(timeline);
  assert.deepEqual([...lines.keys()].sort(), ["e1", "e3"], "只收有合法行的 entryId（空数组不算有行）");
  assert.deepEqual(lines.get("e1"), ["old-1"], "同 id 取第一条（与 applyRenderedLinesByEntryId 同口径）");
  assert.equal(lines.has("e2"), false, "没有 renderedLines 的记录不入表");
  assert.equal(lines.has("e4"), false, "空数组不是合法行（与 validRenderedLines 同判据）");
});

test("主题刷新：sameRenderedLines 按内容逐行比（换色记忆「磁盘赢还是记忆赢」的判据）", () => {
  assert.equal(sameRenderedLines(["a", "b"], ["a", "b"]), true, "逐行相同");
  assert.equal(sameRenderedLines(["a", "b"], ["a", "b", "c"]), false, "长度不同");
  assert.equal(sameRenderedLines(["a", "b"], ["a", "c"]), false, "同长度但内容不同（内容变了必须让磁盘赢）");
  assert.equal(sameRenderedLines(undefined, ["a"]), false, "没有行");
  assert.equal(sameRenderedLines("a", ["a"]), false, "不是数组");
});
