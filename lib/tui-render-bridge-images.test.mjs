...
test("主题补了 pi-tui 的 fallbackColor：Image 的降级分支不再抛错（整段渲染不会被吞）", async () => {
  const { Theme } = await import("@earendil-works/pi-coding-agent");
  setPiThemeConstructor(Theme);
  const { loadPiTheme } = await jiti.import("./tui-render-bridge.ts");
  const theme = loadPiTheme();
  assert.equal(typeof theme.fallbackColor, "function", "pi-tui 的 Image 组件会调它；SDK 的 Theme 自己没有");
  assert.equal(theme.fallbackColor("[image/png]"), "[image/png]", "原样返回（不上色），不抛错");
  setPiThemeConstructor(null);
});
