...
test("GET models.json 损坏时抛 ModelsConfigReadError（不降级为空配置）", () => {
  withDir((dir) => {
    const modelsPath = join(dir, "models.json");
    writeFileSync(modelsPath, "{ invalid json", "utf8");
    assert.throws(() => getSanitizedModelsConfig(modelsPath), ModelsConfigReadError);
  });
});

test("GET 接受 pi 能加载的宽松 JSON（BOM / // 注释 / 尾逗号）", () => {
  withDir((dir) => {
    const modelsPath = join(dir, "models.json");
    writeFileSync(
      modelsPath,
      "\uFEFF{\n  // 注释行\n  \"providers\": {\n    \"loose\": { \"api\": \"openai-completions\", },\n  },\n}\n",
      "utf8",
    );
    const result = getSanitizedModelsConfig(modelsPath);
    assert.deepEqual(Object.keys(result.providers), ["loose"]);
    assert.ok(result.baseline, "可读文件返回基线");
  });
});

test("PUT 拒绝写回读不出的文件，且不改动原字节", () => {
  withDir((dir) => {
    const modelsPath = join(dir, "models.json");
    const broken = "{ invalid json";
    writeFileSync(modelsPath, broken, "utf8");
    assert.throws(
      () => saveModelsConfig(modelsPath, { providers: {} }, null),
      ModelsConfigReadError,
    );
    assert.equal(readFileSync(modelsPath, "utf8"), broken, "原文件字节不变");
  });
});

test("PUT allowUnreadableCurrent：JSON 原文模式可以把坏文件改回来", () => {
  withDir((dir) => {
    const modelsPath = join(dir, "models.json");
    writeFileSync(modelsPath, "{ invalid json", "utf8");
    const result = saveModelsConfig(
      modelsPath,
      { providers: { fixed: { api: "openai-completions" } } },
      null,
      { allowUnreadableCurrent: true },
    );
    assert.equal(result.success, true);
    assert.match(readFileSync(modelsPath, "utf8"), /"fixed"/);
  });
});
