...
test("A2: JSONL 行解析只取 user/assistant 文本（system 系统提示词不进索引）", () => {
  assert.deepEqual(
  ...
  // Pi 0.86 起系统提示词以 role:"system" 条目落盘：它不该被搜到。
  assert.equal(extractSearchableTextFromJsonlLine(JSON.stringify({
    type: "message",
    id: "sys1",
    message: { role: "system", content: "You are a helpful coding agent" },
  })), null);
});
...
test("A2: JSONL 命中带 entryId（可定位）；hermes 路径不给 entryId", () => {
  ...
  const parsed = extractSearchableTextFromJsonlLine(hitLine);
  assert.equal(parsed.entryId, "entry-42", "JSONL 扫描的命中必须给出可定位的 entry id");
  assert.equal(parsed.messageId, "entry-42");

  // FTS/LIKE 查询必须排除 system：否则系统提示词里的常见词会把一票会话顶上来
  // （FTS 的 MATCH 与回退 LIKE 各自的 SQL；回退 MATCH 复用同一条 SQL，所以是 2 条）
  const source = readFileSync(fileURLToPath(new URL("./session-fulltext-search.ts", import.meta.url)), "utf8");
  const queries = source.match(/FROM (message_fts|messages)[\s\S]*?LIMIT \?/g) ?? [];
  assert.equal(queries.length, 2, "应覆盖 FTS 与 LIKE 两条查询");
  for (const q of queries) {
    assert.match(q, /role <> 'system'/, `查询缺少 system 过滤：${q.slice(0, 60)}`);
  }
});
