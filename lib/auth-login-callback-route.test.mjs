...
test("取消路径仍接线：SSE 收尾对挂起记录调 reject 并删除", () => {
  ...
  // 只匹配 `provider,` 会命中 route.ts:183 的 `login(provider,` 之类无关行，删掉记录里的字段也照样绿。
  // 这里锚到挂起记录本身：registry.set(token, { provider, … })。
  assert.match(
    ROUTE_SOURCE,
    /registry\.set\(token,\s*\{\s*provider,/s,
    "挂起记录必须带 provider（POST 才能校验归属）",
  );
});
