...
// 渲染桥 import 了同目录的纯模块（lib/kitty-image），无扩展名相对 import 要 jiti 解析。
const jiti = createJiti(import.meta.url, { tsconfigPaths: true });
const bridge = await jiti.import("./tui-render-bridge.ts");
const { setPiThemeConstructor } = bridge;
...
test("自检早于任何一次渲染也不报假告警（verify 先装槽位，再跑探针）", async () => {
  const bridge = await jiti.import("./tui-render-bridge.ts");
  const { renderDiff } = await import("@earendil-works/pi-coding-agent");

  // 复现审查里的顺序：宿主在 bindExtensions 期间自检，此时还没人调用过 loadPiTheme()
  // （槽位是空的）。旧实现在这里把探针的异常缓存成 false，并给用户报「diff 画不出来」。
  bridge.resetPiThemeForTests();
  bridge.resetSdkThemeVerificationForTests();
  bridge.resetRenderBridgeWarningsForTests();
  delete globalThis[SDK_THEME_KEY];
  delete globalThis[SDK_THEME_KEY_LEGACY];
  const warnings = [];
  ...
  assert.equal(bridge.verifySdkGlobalTheme(), true, "自检必须先装槽位再判定，不能把「还没装」当成「装不进去」");
  assert.deepEqual(warnings, [], `槽位可用时不得报「diff 画不出来」，实际：${JSON.stringify(warnings)}`);
  assert.equal(globalThis[SDK_THEME_KEY], bridge.loadPiTheme(), "自检之后槽位应已就位");
});

test("自检失败不钉死：注入类之后能转好（不再永久 false）", async () => {
  const bridge = await jiti.import("./tui-render-bridge.ts");
  const { Theme: SdkTheme, renderDiff } = await import("@earendil-works/pi-coding-agent");
  bridge.resetPiThemeForTests();
  bridge.resetSdkThemeVerificationForTests();
  bridge.resetRenderBridgeWarningsForTests();
  ...
  assert.equal(bridge.verifySdkGlobalTheme(), null, "主题实例类还没注入：无法判定，返回 null（不缓存 false）");
  bridge.setPiThemeConstructor(SdkTheme);
  assert.equal(bridge.verifySdkGlobalTheme(), true, "注入之后必须能自检通过，而不是沿用上一次的失败");
});
