 ...
 *
 * 生产 import 边界：
 * 1. server-only adapter：sdk-session-host / web-extension-ui / pi-session-io /
 *    project-trust（trust.json 的唯一写入方，必须用 SDK 的 ProjectTrustStore
 *    才能和 pi CLI 共用同一把锁）静态 import
...
const SDK_ALLOWLIST = new Set([
  ...
  // trust.json 的唯一写入方：必须走 SDK 的 ProjectTrustStore，才能与 pi CLI
  // 共用同一把 proper-lockfile 锁（自己读写 JSON 会与终端里的 pi 并发覆盖）。
  // 仅服务端使用（/api/preferences 侧栏加入/关闭项目时同步）。
  join(ROOT, "lib", "project-trust.ts"),
]);
