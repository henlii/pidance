...
// 能力提示只由宿主自己发：插件调的 notify 不在这个数组里，所以这条函数
// 永远不会把插件的一次性通知变成"每次开页面都重弹"。
test("pickCapabilityNotices：插件的 notify 形状不被接受", () => {
  assert.deepEqual(
    pickCapabilityNotices([{ type: "extension_ui_request", id: "n1", method: "notify", message: "插件通知", notifyType: "info" }]),
    [],
    "插件通知没有 notifyType:warning 这一宿主提示标记，不得被重放",
  );
});
