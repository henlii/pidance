...
test("widget 的 interactive 随事件进状态，行内容不变也要更新（issue #103）", () => {
  const off = applyExtensionUiRequest(base, request("setWidget", { widgetKey: "w", widgetLines: ["同"] }));
  assert.equal(off.state.widgets[0].interactive, false);

  // 同一份行内容、只是组件补上了 handleMouse：必须换新状态对象，否则前端一直读到旧的 false
  const on = applyExtensionUiRequest(off.state, request("setWidget", { widgetKey: "w", widgetLines: ["同"], widgetInteractive: true }));
  assert.equal(on.state.widgets[0].interactive, true);
  assert.notEqual(on.state, off.state, "交互性变化必须产生新状态");

  // 反过来（组件不再交互）同样要更新
  const back = applyExtensionUiRequest(on.state, request("setWidget", { widgetKey: "w", widgetLines: ["同"], widgetInteractive: false }));
...
test("折叠思考标签要能从状态快照水合（源码契约）", () => {
  ...
  const calls = src.match(/applyExtensionHiddenThinkingLabel\(/g) ?? [];
  assert.ok(calls.length >= 4, `定义 + 三条水合路径都要用（实际 ${calls.length} 处）`);
