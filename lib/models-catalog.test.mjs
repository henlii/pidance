...
test("buildModelsDataFromDisk 默认模型与 thinking", () => {
  ...
  try {
    ...
    // off:null 禁用；low 自定义；其余 omit（含 xhigh/max）可用
    assert.deepEqual(data.thinkingLevels["p1:m1"], ["minimal", "low", "medium", "high", "xhigh", "max"]);
...
test("thinkingLevelsFor：省略含 xhigh/max，仅 null 禁用", () => {
...
test("buildModelsDataFromCatalog 含内置模型 thinking", () => {
  ...
  assert.deepEqual(data.thinkingLevels["deepseek:deepseek-v4-flash"], ["off", "high", "xhigh", "max"]);
