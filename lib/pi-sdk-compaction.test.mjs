...
test("Pi 0.84.4：大工具结果跨阈值时先压缩再继续模型调用", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    const entries = host.inner.sessionManager.getEntries();
...
test("Pi 0.84.4：压缩失败后运行收敛，截断摘要不落盘，JSONL 可重开", { timeout: 30_000 }, async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-compact-fail-cwd-"));
  const agentDir = mkdtempSync(join(tmpdir(), "pi-compact-fail-agent-"));
  const sessionDir = mkdtempSync(join(tmpdir(), "pi-compact-fail-sessions-"));
  writeFileSync(join(cwd, "large.txt"), "tool-result ".repeat(1800));
  const provider = "compaction-fail";
  const modelId = "pidance-compaction-fail";
  const requests = [];
  const server = createServer(async (request, response) => {
    let raw = "";
    for await (const chunk of request) raw += chunk;
    const body = JSON.parse(raw);
    const hasTools = Array.isArray(body.tools) && body.tools.length > 0;
    requests.push({ kind: hasTools ? "main" : "summary" });
    if (!hasTools) {
      response.statusCode = 500;
  ...
  try {
    ...
    const events = [];
    host.onEvent((event) => {
      if (event.type === "agent_settled" || event.type === "compaction_end" || event.type === "prompt_error") {
        events.push(event);
      }
    });
    await host.send({ type: "prompt", message: "Read large.txt, then answer done." }).catch(() => undefined);
    await host.send({ type: "abort_compaction" }).catch(() => undefined);
    await host.send({ type: "abort" }).catch(() => undefined);
    await waitFor(
      () => !host.isRunning() || events.some((event) => event.type === "agent_settled" || event.type === "prompt_error"),
      "failed compaction did not settle",
      15_000,
    );
    assert.ok(host.sessionFile);
    const reopened = openSessionManager(host.sessionFile, sessionDir);
    ...
    if (compaction) {
      assert.ok(typeof compaction.summary === "string" && compaction.summary.length > 0, "截断摘要不得落盘");
    }
