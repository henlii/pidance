...
test("Pi 0.87.0：大工具结果跨阈值时先压缩再继续模型调用", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    // 0.87.0 实测：这里会多出一轮 summary。原因不是「agent_end 之后新增了压缩检查」
    // ——0.85.1 的 _handlePostAgentRun 同样调用 _checkCompaction——而是 0.87.0 的上下文
    // 估算改为基于 SessionManager 投影：压缩后基准被丢弃时走 chars/4 全量估算，
    // 使这个「大工具结果 ≈ keepRecentTokens」的夹具再次越过阈值。
    // 关键约束见下方「重开后仍保留工具结果」断言：多压一轮不得把本该保留的工具输出摘掉。
    assert.deepEqual(requests.map((entry) => entry.kind), ["main", "summary", "main", "summary"]);
    ...
    assert.ok(agentEnd?.contextUsage, "agent_end 应携带压缩后的上下文快照");
    assert.equal(typeof agentEnd.contextUsage.contextWindow, "number");
    // 测试 provider 不返回 usage，Pi 依约保持压缩后的 token/百分比未知。
    assert.equal(agentEnd.contextUsage.tokens, null);
    assert.equal(agentEnd.contextUsage.percent, null);
    ...
    // 第二轮压缩是收尾性质：不得把本该保留的大工具结果摘掉。
    // 该结果 ≈ keepRecentTokens（5400 token），压缩后必须仍在重开上下文里，
    // 否则用户会静默丢掉工具输出（这是比「多一次摘要」更贵的失败）。
    const reopenedToolResults = context.messages.filter(
      (message) => message.role === "toolResult"
        && JSON.stringify(message.content ?? "").includes("tool-result"),
    );
    assert.equal(
      reopenedToolResults.length,
      1,
      "重开后必须仍能看到 large.txt 的工具结果（第二轮压缩不得摘掉它）",
    );
    // 两轮压缩都必须是阈值压缩（不是 overflow/手动），且不会再来第三次。
    assert.equal(compactionEvents.length, 4, "压缩事件应为两轮 start/end");
    assert.equal(compactionEvents[2]?.type, "compaction_start");
    assert.equal(compactionEvents[2]?.reason, "threshold");
    assert.equal(compactionEvents[3]?.type, "compaction_end");
    assert.equal(compactionEvents[3]?.reason, "threshold");
...
test("Pi 0.87.0：压缩失败后运行收敛，截断摘要不落盘，JSONL 可重开", { timeout: 30_000 }, async () => {
...
test("Pi 0.87.0：压缩中止不落盘，JSONL 可重开", { timeout: 30_000 }, async () => {
  let holdSummary;
...
test("Pi 0.87.0：手动压缩走 summary 且 JSONL 可重开", { timeout: 30_000 }, async () => {
  let mainCall = 0;
  await withCompactionFixture("manual", async ({ response, hasTools, requests }) => {
    ...
    await pendingPrompt;
    // 新 idle 语义：无端点订阅且 settled、空队列时 host 释放（30s/测试 500ms）。
    // run 的 finally 会再退一次，幂等。
    unlisten();
...
test("Pi 0.87.0：上下文窗口过小触发 threshold 压缩且 JSONL 可重开", { timeout: 30_000 }, async () => {
  let mainCall = 0;
...
test("Pi 0.87.0：provider context_length_exceeded 触发 overflow compaction", { timeout: 30_000 }, async () => {
  let mainCalls = 0;
