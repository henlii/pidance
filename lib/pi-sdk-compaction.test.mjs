...
test("Pi 0.85.1：大工具结果跨阈值时先压缩再继续模型调用", { timeout: 30_000 }, async () => {
...
test("Pi 0.85.1：压缩失败后运行收敛，截断摘要不落盘，JSONL 可重开", { timeout: 30_000 }, async () => {
...
test("Pi 0.85.1：压缩中止不落盘，JSONL 可重开", { timeout: 30_000 }, async () => {
  let holdSummary;
...
test("Pi 0.85.1：手动压缩走 summary 且 JSONL 可重开", { timeout: 30_000 }, async () => {
  let mainCall = 0;
...
test("Pi 0.85.1：上下文窗口过小触发 threshold 压缩且 JSONL 可重开", { timeout: 30_000 }, async () => {
  let mainCall = 0;
...
test("Pi 0.85.1：provider context_length_exceeded 触发 overflow compaction", { timeout: 30_000 }, async () => {
  let mainCalls = 0;
