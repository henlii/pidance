...
async function withCompactionFixture(name, handleRequest, run, options = {}) {
  ...
  try {
    ...
    try {
      await run({ host, events, unlisten, requests, sessionFile: host.sessionFile, sessionDir, provider, modelId });
    } finally {
      unlisten();
    }
  } finally {
...
test("Pi 0.84.4：手动压缩走 summary 且 JSONL 可重开", { timeout: 30_000 }, async () => {
  ...
  await withCompactionFixture("manual", async ({ response, hasTools, requests }) => {
    ...
    response.end();
  }, async ({ host, events, unlisten, requests, sessionFile, sessionDir }) => {
    ...
    events.length = 0;
    requests.length = 0;
    const compactPromise = host.send({ type: "compact" }).catch(() => undefined);
    await waitFor(
      () => events.some((event) => event.type === "compaction_end"),
    ...
    assert.ok(requests.some((entry) => entry.kind === "summary"));
    unlisten();
    await waitFor(() => !host.isAlive(), "settled host was not disposed", 15_000);
    const reopened = openSessionManager(sessionFile, sessionDir);
