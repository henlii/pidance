...
test("#104 量行高的宿主：跳过自己那个 line-height: 0 的包装，上跳到父级", async () => {
  const { measurementHostFor } = await jiti.import("./render-width.ts");
  const parent = { tag: "pre" };
  const wrapper = { tag: "span", parentElement: parent };
  assert.equal(measurementHostFor(wrapper), parent, "有父级时量父级（正文行高在那里）");
  const lone = { tag: "span", parentElement: null };
  assert.equal(measurementHostFor(lone), lone, "没有父级时退回自己（至少能量到字体上下文）");
  assert.equal(measurementHostFor(null), null, "没有节点时不量");
});
