...
// issue #76：no-op 不等于「安静地装成让出成功」。插件的典型用法是
// 「让出终端 → spawn 外部编辑器 → 收回」，而这里 spawn 的不是 tty：挂住或失败都
// 无从告知用户，所以 stop() 要报告一次，交给适配器发可见提示。
test("stop() 通过 onUnsupported 报一次（且只报一次），start() 不重复报", async () => {
  const { createHeadlessCustomUiTui } = await loadSubject();
  const reported = [];
  const tui = createHeadlessCustomUiTui(() => {}, 92, 40, {
    onUnsupported: (feature) => reported.push(feature),
  });
  tui.stop();
  assert.equal(reported.length, 1, "stop() 必须报告");
  assert.match(reported[0], /tui\.stop\(\)\/start\(\)/);
  tui.stop();
  assert.equal(reported.length, 1, "同一实例只报一次（插件会反复 stop/start）");
  tui.start();
  assert.equal(reported.length, 1, "start() 不重复报告");
});

test("没有注入 onUnsupported 时 stop() 仍是安全的 no-op（旧调用点不受影响）", async () => {
  const { createHeadlessCustomUiTui } = await loadSubject();
  const tui = createHeadlessCustomUiTui(() => {});
  assert.doesNotThrow(() => {
    tui.stop();
...
test("headless custom UI：terminal 两个维度都是 getter，能反映最新视口", async () => {
  const { createHeadlessCustomUiTui } = await loadSubject();
  let width = 80;
  let rows = 24;
  const tui = createHeadlessCustomUiTui(() => {}, () => width, () => rows);
  ...
  rows = 18;
  assert.equal(tui.terminal.rows, 18, "行数同样是 getter：按它裁切的插件必须拿到真值");
});

test("headless custom UI：仍接受常量尺寸（调用方不用都改成 getter）", async () => {
  const { createHeadlessCustomUiTui } = await loadSubject();
  const tui = createHeadlessCustomUiTui(() => {}, 100, 40);
  assert.equal(tui.terminal.columns, 100);
  assert.equal(tui.terminal.rows, 40);
});
