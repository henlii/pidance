...
const refMedia = (id, role = "model") => ({
  role,
  path: `/tmp/att/${id}.${role}.png`,
  name: `${id}.png`,
  mimeType: "image/png",
  size: 10,
});
...
test("队列里的图片不得被一次「只传正文」的写入抹掉", () => {
  const media = [refMedia("img1", "model"), refMedia("img1", "original")];
  const book = adoptServerSnapshot(EMPTY_BOOK, "A", {
    items: [{ id: "i1", text: "带图", state: "waiting", media }],
    revision: 1,
  });
  // 用户又追加一条：整包替换时旧条目的媒体引用必须原样带回（少了就是丢图）。
  const payloads = payloadsForWrite(queueEntry(book, "A"));
  assert.deepEqual(payloads, [{ text: "带图", media }]);
});

test("乐观载荷的图片计入行投影，且权威条目不再重复显示", () => {
  const { book } = proposeQueue(EMPTY_BOOK, "A", [
    // 模型副本 + 原图 = 用户眼里的一张图。
    { text: "新图", media: [refMedia("img1", "model"), refMedia("img1", "original")] },
    { text: "纯文本" },
  ]);
  const rows = queueRows(queueEntry(book, "A"));
...
test("itemMediaRefs 按条目顺序摊平（取回时逐张回读）", () => {
  const refs = itemMediaRefs([
    { id: "a", text: "x", state: "waiting", media: [refMedia("i1"), refMedia("i1", "original")] },
    { id: "b", text: "y", state: "unknown", media: [refMedia("i2")] },
    { id: "c", text: "z", state: "waiting" },
  ]);
  assert.deepEqual(refs.map((ref) => ref.path), [
    "/tmp/att/i1.model.png",
    "/tmp/att/i1.original.png",
    "/tmp/att/i2.model.png",
  ]);
});

test("只有原图时也数得出一张图（预览与原图同文件的省流写法）", () => {
  const book = adoptServerSnapshot(EMPTY_BOOK, "A", {
    items: [{ id: "i1", text: "带图", state: "waiting", media: [refMedia("i1", "original")] }],
    inFlight: ["在途"],
    revision: 1,
  });
