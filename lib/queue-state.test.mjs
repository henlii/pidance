...
const refImage = (id) => ({ id, path: `/tmp/outbox/${id}.png`, mimeType: "image/png", name: `${id}.png` });

test("队列里的图片不得被一次「只传正文」的写入抹掉", () => {
  const book = adoptServerSnapshot(EMPTY_BOOK, "A", {
    items: [{ id: "i1", text: "带图", state: "waiting", images: [refImage("img1")] }],
    revision: 1,
  });
  // 用户又追加一条：整包替换时旧条目的图片必须按引用带回（否则 Host 侧文件被 sweep 掉）。
  const payloads = payloadsForWrite(queueEntry(book, "A"));
  assert.deepEqual(payloads, [{
    text: "带图",
    images: [{ source: "ref", id: "img1", path: "/tmp/outbox/img1.png", mimeType: "image/png", name: "img1.png" }],
  }]);
});

test("乐观载荷的图片计入行投影，且权威条目不再重复显示", () => {
  const { book } = proposeQueue(EMPTY_BOOK, "A", [
    { text: "新图", images: [{ source: "data", data: "AAAA", mimeType: "image/png" }] },
    { text: "纯文本" },
  ]);
  const rows = queueRows(queueEntry(book, "A"));
  assert.deepEqual(rows.map((row) => [row.text, row.imageCount]), [["新图", 1], ["纯文本", 0]]);
  assert.deepEqual(projection(queueEntry(book, "A")), ["新图", "纯文本"]);
});

test("itemImageRefs 按条目顺序摊平（取回时逐张回读）", () => {
  const refs = itemImageRefs([
    { id: "a", text: "x", state: "waiting", images: [refImage("i1"), refImage("i2")] },
    { id: "b", text: "y", state: "unknown", images: [refImage("i3")] },
    { id: "c", text: "z", state: "waiting" },
  ]);
  assert.deepEqual(refs.map((ref) => ref.id), ["i1", "i2", "i3"]);
});

test("权威条目的图片数量出现在行投影里（在途行没有图片信息）", () => {
  const book = adoptServerSnapshot(EMPTY_BOOK, "A", {
    items: [{ id: "i1", text: "带图", state: "waiting", images: [refImage("i1")] }],
    inFlight: ["在途"],
    revision: 1,
  });
  assert.deepEqual(queueRows(queueEntry(book, "A")).map((row) => [row.text, row.imageCount]), [
    ["带图", 1],
    ["在途", 0],
  ]);
});
