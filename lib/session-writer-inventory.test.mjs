...
const KNOWN_SESSION_WRITERS = [
  {
    ...
    // 树导航（#90）在 Host 活着时用它的 live writer 写：事件必须由本会话 runner 在改动前后
    // 派发，因此不能先交接。此时不交接、不再开磁盘视图；无 live writer 时才走离线写。
    rule: "离线写前 await destroyAsync（树导航在 live 时改用 Host 的 writer）；Pi SessionManager 唯一 JSONL writer",
  },
  {
    ...
    ops: [
      ...
      "append_activity",
      "set_session_name",
      // 树导航（#90）：有 Service 注入时由 Service 用 Host 的 live writer 写；无注入
      // （例如队列恢复拉起的 host）时 Host 用同一份 plan* 就地写。两条路径都是同一个
      // SessionManager，不新增第二个 writer。
      "select_leaf_exact / branch_from_assistant（无 Service 注入时的本地落地）",
    ],
...
test("auto-name fallback：写盘前 destroyAsync 再 renameSession", async () => {
  ...
  const svc = await readFile(new URL("./session-service.ts", import.meta.url), "utf8");
  assert.match(route, /sessionService\.autoNameSession/);
  assert.doesNotMatch(route, /generateSessionTitleFromMessages/);
  assert.match(svc, /async autoNameSession/);
  const destroyIdx = svc.indexOf("await service.destroyAsync(sessionId)");
  const renameIdx = svc.indexOf("await service.renameSession(sessionId, result.title)");
