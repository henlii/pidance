...
/**
 * 顺序门禁：按 entryId 定位跳转时，「进入浏览历史态」必须发生在**加载之前**。
 *
 * 起因（用户反馈）：跳转到未加载的部分时，内容会先向下滚一段，然后才替换成加载后
 * 的内容。成因是 hydrate 提交新窗口的那一帧会改变内容高度，若此时自动跟随仍是
 * following，它会先钉底；而 notifyBrowsingHistory() 原先写在 hydrate 之后，
 * 于是「钉底」发生在切态之前。
 */
test("jumpToEntry：notifyBrowsingHistory 必须在 hydrate 之前调用", async () => {
  const source = await readFile(srcUrl("hooks/useAgentSession.ts"), "utf8");
  const start = source.indexOf("const jumpToEntry = useCallback(");
  assert.ok(start > 0, "未找到 jumpToEntry");
  // 取到本函数的 catch 结束（下一个 `}, [...` 收尾）即可覆盖整个函数体
  const body = source.slice(start, start + 8000);
  const browseIdx = body.indexOf("notifyBrowsingHistory()");
  const hydrateIdx = body.indexOf("registry.hydrate(");
  assert.ok(browseIdx > 0, "jumpToEntry 必须进入浏览历史态（否则视口会被拉回尾部）");
  assert.ok(hydrateIdx > 0, "jumpToEntry 必须 hydrate 定位窗口");
  assert.ok(
    browseIdx < hydrateIdx,
    "notifyBrowsingHistory 必须早于 hydrate：否则提交新窗口那一帧会先被自动跟随钉底",
  );
});
